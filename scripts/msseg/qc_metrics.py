...
def compute_metrics(
    ...
    if mtx_path.exists():
        ...
        if pos_df is not None:
            pos_idx = pos_df.set_index("barcode")[
                ["pxl_row_in_fullres", "pxl_col_in_fullres", "in_tissue"]
            ]
    ...
    for roi in rois:
        ...
        mask_roi = col_vals.between(x0, x0 + w) & row_vals.between(y0, y0 + h)
        ...
        for method in ("nuc", "mcseg"):
            ...
            in_tissue = (
                a.obs.get("in_tissue", pd.Series(1, index=a.obs.index))
                .values[in_bounds]
                .astype(bool)
            )
            ...
            print(
                f"  {name} {method}: cells={n_cells} FTC={ftc:.3f} NED={ned:.3f} coexp={coexp:.4f}"
            )
            del count_mat
            gc.collect()
