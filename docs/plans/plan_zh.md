...
- **Code Promotion 動態升格框架**：旨在根治動態生成代碼生命週期無序膨脹與難以維護之宿疾。系統實時監測臨時分析腳本之重用頻次，一旦特定邏輯之 `reuse_count` 累積達�

```mermaid
graph TB
    %% 1. 定義高階配色樣式 (和諧、專業的 HSL 視覺系統)
    ...
    %% -------------------------------------------------------------------
    subgraph G0["0. 【常態調用與技能升格】(Skill Registry & Promotion)"]
        agent(["Agent / LLM"]):::entry
    ...
    %% -------------------------------------------------------------------
    subgraph G1["1. 【健康監測】變更與異常偵測 (Monitor)"]
...
> [!TIP]
> 💡 **以「生資工具診療室」的大白話，輕鬆理解 HELIX 自主演進四部曲：**
>
> 如果把實驗室裡的「分析工具程式碼」想像成實驗室的高端分析儀器，那麼 **HELIX 就是這套儀器的「智慧監控系統、AI 維修工程師、電子病歷檔案庫，以及中央工具收納箱」**：
>
> 1. **第一步：主動健康監測（監測與追蹤）➔ 亮黃燈**
>    只要開發者將分析程式放進 **「程式碼工具箱（tools 表）」** 中以供使用，或是程式跑得慢、出錯了，系統都會自動記錄。如果工具箱中的某個程式在短時間內被「維修（修改）了 3 次以上」（熱區偵測），系統就會亮起黃燈警告：「這段程式可能寫得不夠好、不夠穩定，需要大修了！」
> 2. **第二步：精密健康檢查（多維度評估）➔ 找病因**
>    亮起黃燈後，系統會自動對這段程式做精密檢查：是邏輯寫得太繞像迷宮（循環複雜度）？還是每次修改都集中在同一個零部件（行級變動 X-Ray 行分析）？藉此找出根本病因。
> 3. **第三步：AI 醫生會診與治療（穩定化迭代閉環）➔ 動手術並歸位**
>    AI Agent 扮演的「資深工程師醫生」會看著這份檢查報告（診斷記錄），擬定維修方案（行動計畫），並動手將程式碼重新重構優化，讓它變得精緻、穩定又好用。**修好後的安全工具會自動「升級版本並重新歸位回程式碼工具箱」**，再次提供大家安全使用！
> 4. **第四步：病歷歸檔與智慧忘卻（記憶與忘卻機制）➔ 留病歷**
>    優化好之後，系統會為這段程式拍一張病歷照（視覺快照）永久保存，讓未來的醫生可以隨時翻閱。但為了不佔大腦空間，就像人類記憶一樣，太久以前的舊病歷照片會被自動「模糊化」降低解析度（降採樣）——既保留了歷史輪廓以供未來參考，又不會讓大腦塞車。由提供最優化之量化數據依據。(詳細診療評估與視覺快照 Bilinear 降採樣決策見 [附錄 A7. HELIX 系統結構與自適應演化閉環](#A7-HELIX))。

- **雙軌記憶協同**：ENGRAM 記錄之溯源鏈能精確鎖定產出所對應之工具 content-hash；當 HELIX 辨識出某工具因修改頻繁而設計不穩（進入熱區）時，能瞬間反查並標記該版本產出之所有歷史成果，使用戶在進行跨樣本比對時，得以清晰判別差異究竟源於生物學參數之擾動，抑或是工具代碼本身之版本漂移。(詳細語意 HNSW 與 3-way RRF 決策見 [附錄 A8. ENGRAM 產出物索引與記憶檢索](#A8-ENGRAM))。
...
為保障 L2 銀層中分析工具的長期健康度與版本演化可追溯性，系統首創之 **HELIX（Health-Evolving Loop with Iterative eXpiration）** 模組採用「先理解概念、後分析架構」之雙軌視覺化流程，實施「使用/變更 ➔ 監測 ➔ 評估 ➔ 診斷穩定化 ➔ 雙軌記憶忘卻」之生命週期治理：

#### 【L1 級】高階概念演進環（5 秒大白話讀懂）
...
> 💡 **以「生資工具診療室」的大白話，輕鬆理解 HELIX 自主演進四部曲：**
>
> 如果把實驗室裡的「分析工具程式碼」想像成實驗室的高端分析儀器，那麼 **HELIX 就是這台儀器的「智慧監控系統、AI 維修工程師與電子病歷檔案庫」**：
>
> 1. **第一步：主動健康監測（監測與追蹤）➔ 亮黃燈**
>    只要開發者修改了某個分析程式，或是該程式跑得慢、出錯了，系統都會自動記錄。如果某個程式在短時間內被「維修（修改）了 3 次以上」（熱區偵測），系統就會亮起黃燈警告：「這段程式可能寫得不夠好、不夠穩定，需要大修了！」
> 2. **第二步：精密健康檢查（多維度評估）➔ 找病因**
>    亮起黃燈後，系統會自動對這段程式做精密檢查：是邏輯寫得太繞像迷宮（循環複雜度）？還是每次修改都集中在同一個零部件（行級變動 X-Ray 行分析）？藉此找出根本病因。
> 3. **第三步：AI 醫生會診與治療（穩定化迭代閉環）➔ 動手術**
>    AI Agent 扮演的「資深工程師醫生」會看著這份檢查報告（診斷記錄），擬定維修方案（行動計畫），並動手將程式碼重新重構優化，讓它變得精緻、穩定又好用。
> 4. **第四步：病歷歸檔與智慧忘卻（記憶與忘卻機制）➔ 留病歷**
>    優化好之後，系統會為這段程式拍一張病歷照（視覺快照）永久保存，讓未來的醫生可以隨時翻閱。但為了不佔大腦空間，就像人類記憶一樣，太久以前的舊病歷照片會被自動「模糊化」降低解析度（降採樣）——既保留了歷史輪廓以供未來參考，又不會讓大腦塞車。

### 分析技能組成結構（Skill Composition）

HELIX 管理工具的「版本生命週期」；但工具本身是如何組成一個可被 Agent 呼叫的「分析技能」，需要另一套組合規範。

系統中每一個分析領域（如 Bulk RNA-seq、Spatial Transcriptomics、MCseg 視覺化）對應一個**技能工具箱（Skill Toolbox）**，由四個元件協同構成：

| 元件 | 路徑 | 職責 |
|------|------|------|
| **技能說明書（Playbook）** | `playbooks/*.md` | 定義 Agent 在此領域的行為規範：何時呼叫哪個工具、輸出格式、限制條件 |
| **分析函數（Analysis Function）** | `analysis/*.py` | 實際的計算邏輯：讀 L2 Parquet → 計算 → 回傳 base64 報告 |
| **MCP 工具包裝（MCP Tool）** | `server/bio_memory_server.py` | 把分析函數包裝成 Agent 可呼叫的工具介面（帶參數 schema、錯誤處理） |
| **HELIX 版本登記（register_tool）** | `analysis/tool_registry.py` | 將工具的版本、hash、修改原因寫入 `tools` 帳本，啟動健康追蹤 |

四元件缺一不可：

- 沒有 **Playbook**：Agent 不知道何時呼叫此工具，或會用錯參數
- 沒有 **Analysis Function**：工具無計算實體，僅有 API 殼
- 沒有 **MCP Tool**：Agent 無法直接呼叫（須走 `bio_execute_code` 動態碼，不穩定）
- 沒有 **register_tool()**：HELIX 版本帳本空白，stale 分析無法追蹤

```mermaid
graph LR
subgraph skill["一個分析技能（Skill Toolbox）"]
pb["playbooks/*.md\n技能說明書"]
fn["analysis/*.py\n分析函數"]
mcp["bio_memory_server.py\nMCP Tool 包裝"]
reg["register_tool()\nHELIX 版本登記"]
end

agent(["Agent / LLM"]) -->|"讀取行為規範"| pb
agent -->|"呼叫"| mcp
mcp -->|"執行"| fn
fn -->|"每次修改後"| reg
...
每次擴充一個新的分析能力，依序完成以下四步：

1. **建 Playbook**：在 `playbooks/<domain>.md` 定義 Agent 的行為規範（何時呼叫、輸入格式、限制）
2. **寫分析函數**：在 `analysis/<domain>.py` 實作計算邏輯，遵循 inline base64 圖片輸出規則
3. **注冊 MCP 工具**：在 `server/bio_memory_server.py` 的 `call_tool` 中加入對應的 `elif` 分支，並在 `list_tools()` 宣告工具 schema
4. **呼叫 register_tool()**：完成上述三步後，立即執行 `register_tool()` 寫入 HELIX 帳本

> [!WARNING]
> **不可跳過 register_tool()**：步驟 4 常被誤以為是「可選的簿記動作」而省略。但跳過後，`analysis_history.tool_id` 將為 NULL，HELIX 的 stale 分析追蹤失效，等同讓整個版本治理機制在這個工具上盲目運行。
...
系統的「自動化運維」與「記憶維護」皆依賴一組背景常駐的排程任務（Scheduler Tasks），共有 5 個核心定時任務在背景執行，以維護系統健康度與防範斷電損壞風險。

### 定時排程配置

| 任務腳本 | 執行週期 | 功能與記憶維護職責 |
| :--- | :--- | :--- |
| `scheduler/backup_db.py` | 每日 02:00 | 自動備份 `bio_memory.duckdb`。執行 `EXPORT DATABASE` 落盤至 APFS 安全區 `~/bio_db_backups/`（保留最近 7 天），確保極端斷電下亦可 100% 還原。 |
| `scheduler/cleanup_l1_cache.py` | 每日 03:30 | 自動掃描並清理 L1 記憶快取。`DELETE FROM memory_recent WHERE expires_at < now()`（TTL 預設 7 天）。 |
| `scheduler/rebuild_hnsw.py` | 每週日 03:00 | HNSW 向量索引維護。由於 HNSW 不支援增量更新，每週日自動對 L1 與 L2 向量進行 `DROP + CREATE INDEX` 重建，防止索引碎片化並提升語意搜尋效率。 |
| `scheduler/scan_new_samples.py` | 每 30 分鐘 | 主動式實體註冊。自動掃描結果目錄，若有新樣本產出，即刻自動登記至 `sample_registry` 中。 |
| `scheduler/helix_expire_snapshots.py` | 每週日 04:00 | HELIX 遺忘曲線空間降採樣。針對已關閉的歷史體檢快取進行漸進式降採樣（320p ➔ 160p），降低 vision token 開銷。 |

### 手動備份與還原機制

除了每日定時自動執行外，系統亦提供手動備份與還原工具。開發者可直接於終端機執行：
...
# 從最新的安全備份中復原資料庫
.venv/bin/python scheduler/backup_db.py --restore
```

### 常駐服務與啟動管理（launchd plist）
...
```mermaid
graph TD
subgraph WRITE["① 分析完成時（自動觸發）"]
analysis["生物資料分析<br/>bulk RNA / Spatial / DEG…"]
reg["register_artifact()<br/>artifact_registry.py"]
embed_fn["embed.py<br/>bge-m3 1024-dim → 語意向量"]

analysis -->|"分析完成後自動呼叫"| reg
reg -->|"生成語意向量"| embed_fn
end
