...
為了根治動態生成代碼在生產環境中的「生命週期無序膨脹與幻覺安全漏洞」，Evo_PRISM 首創了 **HELIX (Health-Evolving Loop with Iterative eXpiration)** 動態升格框架。其運作流程如下：

1.  **動態生成與沙盒運行 (Ad-hoc Execution)**：當 Agent 面臨全新的複雜查詢時，生成臨時代碼腳本，並在配置有嚴格 imports 白名單與 timeout（60 秒）的安全沙盒中實體運行。
2.  **重用偵測與 Code Promotion**：系統實時監測臨時分析腳本的重用頻次。一旦某個 Ad-hoc 邏輯之 `reuse_count` 累積達到閾值（$\ge 3$），自動啟動 **Code Promotion** 機制：AI Agent 將該代碼進行系統化的軟體工程重構優化（降低 Radon 循環複雜度，消除重複代碼），將其「晉升」為 `analysis/` 下的標準工具模組。
3.  **動態註冊與熱加載**：重構後的工具自動調用 `register_tool()` 登記至 `tools` 版本履歷表，並生成內容特徵 SHA256 哈希。接著，MCP Server 重新加載 (Hot-reloading) 新工具，使其成為 Agent 能夠在後續對話中以極低 Token 開銷直接呼叫的「標準工具 (Playbook / MCP Tool)」。
4.  **熱區健康檢測與忘卻**：若某個標準工具在日常修改中 Churn Ratio（行級變動率）過高，熱區偵測器會亮黃燈，啟動 AI 醫生重構會診。優化好後，保存視覺記憶（Bilinear 降採樣快照），實現長期記憶的智慧衰減。
