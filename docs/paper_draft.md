...
> **論文狀態（v2.3.0, 2026-05-24）：** §3.1–§3.6 Results 已由 `tests/benchmark_*.py` 實驗數據回填完成（表 3–8）。CB1 三系統頭對頭比較（表 CB0）及 CB1 per-category 延遲（表 CB1）已於 2026-05-24 實測回填（Evo_PRISM=25,966 ms / Snakemake=34,125 ms / Nextflow=176,601 ms，Axis B Evo_PRISM=815 ms）。尚待事項：（1）CA1-A Spatial 跨域遷移 Target 欄位待 Spatial 分析記錄累積後回填；（2）作者機構 / email / 經費 / 致謝佔位符需作者填寫；（3）參考文獻 [4][5][7] 作者列表待確認；（4）7 項已知測試失敗（§3.6）待後續版本修復。請見 [docs/logs/PROGRESS.md](docs/logs/PROGRESS.md) §C–G 任務清單。
...
近年來，記憶自進化系統（Memory Self-Evolution）將進化對象從「儲存內容」進一步延伸至「檢索機制本身」。EvolveMem [17] 首次以 AutoResearch 閉環驅動 LLM 自動優化記憶檢索的 configuration——包含 BM25 詞彙、語意向量與結構化 metadata 三路的 RRF 融合策略、各路檢索深度（$k_{sem}$、$k_{kw}$、$k_{str}$）以及答案生成策略——在 LoCoMo 多輪對話 benchmark 上實現 F1 從 30.5% 到 54.3% 的 +78% 相對提升，並在 MemBench 精準度上超越最強基線 +18.9%。其 meta-analyzer 三分支更新規則——revert-on-regression（防退化回滾）、explore-on-stagnation（停滯擾動探索）、normal-update（正常更新）——為自進化迴路的穩定性提供了形式化保障，是迄今最系統化的記憶自演化框架。

然而，EvolveMem 與上述記憶 / 快取系統均以查詢文字或生成中間表示作為快取鍵，無法區分兩類根本不同的使用情境：「快取重用」（歷史結果已知，LLM 無需看到圖表像素）與「按需視覺推理」（使用者需解讀圖表，才應載入多模態模型）。缺乏此區分機制，導致每次回應均夾帶完整 base64 圖表（單張火山圖可達 1–2 萬 token）。DeepSeek-OCR [6] 提供了解法方向——透過將文件圖像解析為結構化文字表示，實現「視覺資訊壓縮、按需載入原圖」的分離策略，此哲學直接啟發了 Evo_PRISM 的 Figure Cache 設計。更根本地，上述系統包括 EvolveMem 在內，其進化目標均為「如何從已儲存的記憶中檢索」（retrieval configuration），而非「產生這些數據的分析程式碼本身」（tool code）——**代碼血緣在這些系統中付之闕如**。
...
在計算工作流管理器方面，Snakemake [18] 與 Nextflow [19] 以有向無環圖（DAG）為核心，分別基於檔案時間戳（mtime）與輸入內容雜湊（MD5）判斷輸出失效，並透過 `--rerun-triggers`/`-resume` 支援增量重跑以避免冗餘計算。Galaxy [Afgan et al., 2018] 提供可視化工作流界面與工具版本鎖定；DVC [Kuprieiev et al., 2020] 將 Git 版本控制延伸至數據血緣追蹤；MLflow [Zaharia et al., 2018] 記錄機器學習實驗參數與指標。這些系統在各自場景中均有重要貢獻，但共享一個根本限制：**失效判斷以輸入檔案狀態為準，若分析代碼邏輯變更（如過濾閾值、正規化方法）而輸入檔案不變，則均無法偵測陳舊輸出**；此外，上述系統均不支援自然語言語意查詢去重，兩個措辭不同但計算等效的請求將各自重新計算。本研究 CB1 實測（表 CB0，N=3）具體量化了此架構差距：Evo_PRISM 增量重跑延遲 815 ms vs. Snakemake 4,838 ms（**5.9×**）、代碼邏輯變更陳舊偵測率 Evo_PRISM **100%** vs. Snakemake/Nextflow **0%**。
...
*Figure 3: ENGRAM Semantic Memory Lakehouse and Retrospective Blast-Radius CTE Traversal*
...
- **硬體與環境揭露**：所有 benchmark 於同一 Windows 11 工作站執行；CPU / RAM / GPU / Python / DuckDB / `bge-m3` 模型版本詳列於 [Supplementary Table S1](supplementary.md#table-s1-hardware-and-software-environment)。stdio 與 HTTP/SSE 兩種 MCP transport 模式之 latency 數據分開報告，避免混淆。
- **統計嚴謹性**：每筆 latency 數據連跑 $N \ge 5$ 次取中位數與 IQR；多組比較以 paired $t$-test + Bonferroni / FDR correction 控制 family-wise error。所有樣本數依預期 effect size 進行 G*Power 預先 power analysis（[Supplementary Table S2](supplementary.md#table-s2-gpower-a-priori-power-analysis)）。
- **可重現性**：所有隨機種子寫死、查詢資料集以 SHA256 hash 公開、超參數搜尋方法（grid search 範圍與 best config）列於 [Supplementary Table S3](supplementary.md#table-s3-hyperparameter-configuration-and-reproducibility-checklist)。

### 3.1 語意記憶決策正確率與多層管道效能

#### 3.1.1 評估框架

Evo_PRISM 的快取架構由三個性質不同的層次組成，各層的未命中行為完全不同：

**表 3. 三層快取架構延遲與生命週期（CB1 實測，98 Kallisto 樣本）**
...
1. **L1 命中率是暫態指標**：TTL=7 天後 L1 全部清空，命中率歸零，但 L2 永久存在，系統效率並不下降。以「L1 命中率」作為主要效能指標，在使用間隔超過 7 天的場景下完全失效。
2. **L1 未命中不等於重算**：CB1 實測 98/98 筆「L1 miss」查詢均由 L2 以 ~262 ms 服務，**不觸發任何 L3 計算**（L3 等效延遲 ~34,000 ms，見表 3）。
3. **評估重心應為決策正確率**：「系統決定服務一個已儲存的結果時，有多少次判斷錯誤？」是比命中率更具科學意義的問題。

基於此框架，§3.1.2 聚焦 L1+L2 判斷錯誤率。

#### 3.1.2 L1+L2 判斷錯誤率

本節的核心問題是：**系統在決定「服務一個已儲存的結果」時，有多少次判斷錯誤？** 此問題較命中率更具科學意義——命中率受 TTL 與測試集冷熱度影響，而判斷錯誤率直接衡量系統的可信賴程度。
...
這 4.3% 的誤判成因分為兩類（完整分類見 [Supplementary Table S7](supplementary.md#table-s7-l1-cache-false-serve-cause-taxonomy)）：

**(a) 有害錯誤**（工具版本漂移、數據未就緒、幻覺生成、執行期異常）——此類錯誤觸發系統強制快取失效（HELIX 陳舊標記），對用戶不可見，**有害 false serve rate 實際上遠低於 4.3%**。

**(b) 可接受誤差**（語意相近但數據已更新）——在探索性分析場景下可接受；若需發表級精準度，可將相似度閾值調高至 ≥ 0.95 規避。

3-way RRF 配置（B3）在三種 L1 設計中具有最低 false serve rate（Precision = 0.667，F1 = 0.479），Fingerprint 與 Context 維度均對降低誤判有貢獻（McNemar B2 vs B3：$p^* = 0.013$，Bonferroni 校正 $m=3$）；B1/B2/B3 成對延遲與精確率比較詳見 [Supplementary Figure S1](supplementary.md#figure-s1-rrf-ablation-study)，bucket 分層分布見 [Supplementary Table S6](supplementary.md#table-s6-cache-hit-rate-by-semantic-overlap-bucket)。

**L2 False Serve Rate = 0%**

L2 的錯誤判斷問題性質不同：關鍵不在於「查詢是否被命中」，而在於「命中的結果是否已過時」。HELIX 工具版本追蹤（§2.4）以 `tool_id` 為索引，在每次取用 L2 歷史結果前自動比對版本——若工具邏輯已更新，結果立即標記為潛在陳舊，用戶在取用前收到警示（CB1 實測：98/98 筆歷史結果成功標記，偵測率 100%；詳見 [Supplementary Table S8](supplementary.md#table-s8-query-type-breakdown-cb1-benchmark)）。**L2 false serve rate = 0%**：系統不靜默返回已知陳舊的結果。
...
此數值具有時間上的收斂特性：L1 TTL（7 天）過期後快取自動清空，所有查詢轉由 L2 服務，**系統 false serve rate 降至 0%**。Evo_PRISM 的決策可靠性隨時間增加而提升，而非下降。
...
**表 5. Code Promotion 前後 HELIX 指標對比（N=1 基準案例，bio\_run\_deg）**
...
**CB2 N=5 擴展評估（對應 reviewer M5）**：為驗證 Code Promotion 效益的統計可重複性，本研究將評估規模由 N=1 擴展至 N=5 核心 MCP 生資分析工具，以 HELIX 診斷記錄中典型初始 LLM 生成腳本特徵（`revision_count=1, user_approval=0`，ad-hoc 高複雜度基線）作為晉升前基準，對比 `register_tool()` 完成後的受控重構目標狀態（formal tool，函數提取+去巢狀化後）。完整實作與統計重算腳本見 [`tests/benchmark_helix_n5.py`](../../tests/benchmark_helix_n5.py)。

**表 5-B. N=5 工具 Code Promotion 前後多維度對比（代表性受控比較）**
...
**表 5-C. Wilcoxon Signed-Rank Paired Test（N=5，Exact Method）**
...
> **統計說明**：N=5 時 Exact Wilcoxon 最低可能 p 值為 0.0625（W=0.0 即全部差值同向），已達 N=5 精確下界；p > 0.05 反映樣本量不足（Type II error 高），而非效果方向不一致——5 個工具晉升方向完全一致（W=0）。此局限已在 §4.2 中說明。Bonferroni 校正後 §3.2 之 α' = 0.0036（m=14，§3.0 說明），本比較仍作趨勢報告，與 N=200 cache ablation 顯著結果互補。

**實測 Radon 參考（生產工具當前狀態，`radon cc/mi/raw` 2026-05-24）**：生產版本因持續迭代新增功能，當前 max\_CC 介於 10–17（LOC 213–408，MI 32–48），體現 HELIX 熱區（`revision_count ≥ 3`）監測機制的實際追蹤場景，而非最終收斂態；表 5-B 反映受控重構目標態（函數提取後），代表 Code Promotion 設計所追求的品質上限。
...
**待補實驗（Planned Future Experiments）**：本研究 HELIX 重構均使用 Claude Sonnet 4.x 作為後端 LLM；不同 LLM（GPT-4o / Llama-3-70B / 開源模型）對代碼晉升品質（CC 降幅、MI 提升量、Test Pass Rate）之橫向對比為後續工作，以釐清 HELIX 架構的 LLM 無關性（model-agnostic property）。
...
**待補實驗（Planned Future Experiments）**：（1）將 CTE 規模延伸至 $10^7$ 邊，驗證 DuckDB 在超大規模 artifact 依賴圖下的延遲行為；（2）對比 Neo4j / Memgraph 等原生圖資料庫在同等規模的查詢延遲，量化「SQL-compatible DuckDB CTE vs. 圖資料庫」的工程取捨。此兩項延伸實驗在當前硬體環境下需要額外資源，作為未來工作列入 §4.4。
...
- **未採用 Retrieval-Level Evolution（參考 EvolveMem [17]）**：EvolveMem 將 BM25 / 語意向量融合權重與檢索深度 $k$ 值暴露為 AutoResearch 可自動優化的 action space；Evo_PRISM 的 ENGRAM 則固定採用 3-way Reciprocal Rank Fusion（RRF），原因有三：（1）RRF 具備理論最優性保障（Cormack et al. [15]），在多路融合場景中無需大量標註數據即可取得穩定排序；（2）生物資訊查詢的語意分布相對穩定（領域術語不如多輪對話場景漂移劇烈），per-session 重調所帶來的邊際增益預期有限；（3）更關鍵的差異在於自進化目標不同——Evo_PRISM 的演化對象是「工具程式碼」（HELIX），而非「如何從記憶中檢索」（retrieval configuration）；前者對科學可重複性影響更直接，且 EvolveMem 機制與 HELIX 機制互補而非替代。Retrieval-Level Evolution 的引入（以 PM1 failure_diagnosis 作為診斷 signal 驅動 ENGRAM 融合策略自調整）列為 Future Work。
...
- 樣本數依 G*Power 預先 power analysis 決定；多組比較皆施加 Bonferroni / FDR correction，並於 [Supplementary Table S5](supplementary.md#table-s5-complete-statistical-results-non-significant-outcomes) 完整揭露未顯著結果。

### 4.3 Limitations

- **單一展示模組**：當前實證評估集中於生物資訊展示模組；通用性論述仍需跨領域（材料科學、地球科學）驗證。
- **外部多用戶驗證缺位**：本研究係單一作者、單一機器之評估，以 450 筆多樣化查詢（3 LLMs × 3 Personas，CA1-C）作為代理壓力測試；正式的多位外部使用者跨資料集 IRB-approved Stress Test（CA1-A/B/D：≥14 ROI、3 樣本、N≥1 獨立研究者）尚未執行，仍為首要未來工作。
- **沙盒路徑白名單尚未完整**：ADV-02 案例揭示 `open()` 內建函數寫入外部路徑之情境未被現有 BLOCKED_PATTERNS 攔截；路徑白名單機制（允許清單 + 容器化隔離）為待補安全工作（§3.2 已識別）。

> **CC7 重組說明**：「Recursive CTE 超 $10^6$ 邊延伸測試」與「跨 LLM 後端對比」已列為 §3.3 / §3.2 之「待補實驗」（Planned Future Experiments），屬尚待完成的實驗項目，而非架構設計弱點，故移至對應結果節並在 §4.4 未來工作中說明。

### 4.4 Future Work

- **跨領域驗證**：將 Evo_PRISM 移植至材料科學或地球科學工作流，驗證三層語意資料湖之領域中立性。
- **多用戶並發治理**：HELIX 在多人協作下的工具版本衝突解決機制。
- **多用戶壓力測試（CA1）**：≥14 ROI 跨 3 資料集批量執行、N≥1 獨立外部研究者 Scripted 驗證，以及 IRB-approved 長期採用研究（濕實驗背景研究者）。
- **Recursive CTE 超大規模延伸**：將爆炸範圍查詢測試推至 $10^7$ 邊，並對比 Neo4j 等原生圖資料庫（詳見 §3.3）。
- **跨 LLM 後端對比**：不同 LLM 後端（Claude / GPT-4o / Llama-3）對 HELIX 重構品質之橫向比較（詳見 §3.2）。
