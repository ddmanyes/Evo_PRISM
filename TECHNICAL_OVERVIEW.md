...
| 層 | 認知對應 | 特性 |
|:---|:---|:---|
| **L1 Gold** | 工作記憶（Working Memory） | 亞毫秒存取、7 天 TTL、高頻熱點攔截 |
| **L2 Silver** | 情節記憶（Episodic Memory） | 記錄「何時、用什麼工具、做了什麼分析」的永久帳本 |
| **L3 Bronze** | 語意長期記憶（Semantic Long-term Memory） | 不可變的領域知識原始數據，唯讀存取 |

此分層非僅比喻——HELIX 的選擇壓力作用於 L2 帳本（HealthScore < 閾值 → 觸發重構），`bio_find_tool` 的探索從 L1 工具索引出發，三層共同構成系統的「認知架構」，驅動記憶的持續演化。
...
HELIX 是工具治理子系統，追蹤分析工具的每一次修改，並自動化從臨時腳本到生產 MCP 工具的完整生命週期。HELIX 運作於**達爾文式選擇壓力**之下：低重用率（ReuseCount 停滯）與高複雜度（McCabe CC 攀升）構成雙重淘汰壓力——唯有通過沙盒迴歸測試、HealthScore 持續維持在 θ_warning 以上的工具，方能在 `tool_catalog` 中存活並持續被引用；品質下降的版本則自動廢棄下架。**複雜度是環境壓力，重用率是生存優勢**，兩者共同驅動工具庫的定向演化。
...
### 4.6 程式碼生成引導：先搜後寫閉環（bio_find_tool）

HELIX 不只管理工具版本——還主動引導 Agent **決定要不要寫新程式碼**。
...
Agent 收到分析請求
   ↓  0 LLM token
bio_find_tool → HNSW 語意搜尋 tool_catalog
   ├─ 命中（cosine ≥ 0.70）→ 取得 import_hint，直接呼叫現有工具
   └─ 全 miss → bio_execute_code 沙盒動態撰寫
                    ↓ 晉升後
                register_tool() 自動回寫 tool_catalog（正向閉環）
```

`tool_catalog` 只收兩類條目，確保 Agent 看到的都是「健康且有版本追蹤」的程式碼：
- **HELIX 已晉升工具**：`register_tool()` 寫入時自動同步，廢棄版本自動下架
- **`analysis/` 公開函數**：`index_modules()` 回填，源碼指紋不變則跳過重嵌入（冪等）

| 優勢 | 機制 |
|:---|:---|
| **節省 Token** | 搜尋本身 0 LLM token（bge-m3 本機 embedding + DuckDB HNSW）；只回傳 top-K 函數名稱與簽名，不把整份目錄塞 system prompt |
| **寫法一致性** | Agent 拿到 `import_hint` 直接呼叫既有函數，不自行發明邏輯相同但風格不同的程式碼 |
| **品質門控** | catalog 只收 HELIX 認證工具；Agent 被引導呼叫的永遠是當前健康版本，不會用到老舊程式碼 |

**持續自強化飛輪（Self-Reinforcing Flywheel）**

這不是靜態的工具箱——每一次使用都讓它變得更好。這正是「**Evo**_PRISM」名稱中 Evo（Evolutionary）的核心涵義：

```
① 新需求出現 → 沙盒寫碼 → 晉升 → 自動進入 tool_catalog
       ↓
② 同類需求再次出現 → bio_find_tool 命中 → 直接重用（不寫新碼）
       ↓
③ HELIX 持續健康監控 → 不健康版本自動下架 → catalog 品質 ↑
       ↓
④ catalog 覆蓋範圍越廣、工具品質越高
       ↓
① 回到起點：下次寫更少的碼、用更好的工具
```

從第一次使用到第 N 次使用，Agent 從零撰碼的頻率**單調遞減**，可重用工具庫的廣度與健康度**單調遞增**。這個特性是靜態 RAG 系統所缺乏的——Evo_PRISM 的程式碼記憶會隨每次互動演化，而不是停在固定版本。

#### 4.6.1 自強化飛輪縱向演化實證 (R10)
...
![Figure 8: Tool Catalog Flywheel Evolution Curve](docs/images/Figure8_Flywheel_Evolution.png)
*圖：Figure 8. 工具庫自強化飛輪縱向演化與延遲特徵雙面板圖*。(A) 語意搜尋命中率隨工具庫演化規模之單調躍升曲線，實證工具晉升促成之語意記憶累積效應；(B) 查詢搜尋延遲隨規模增長之平坦演化曲線，實證 HNSW 向量檢索之亞毫秒級高擴展性。

> **與 GitNexus 的對比**：GitNexus \[10\] 以 git 歷史引導程式碼生成；`bio_find_tool` 的差異在於引導來源是「HELIX 認證過、有健康分數、有版本追蹤的函數庫」——Agent 不只找到了相關程式碼，還保證它是健康且當前版本的。
...
ENGRAM 形成獨立於 HELIX 的第二條飛輪，兩者互補：
...
## 7. 空間大數據 Ingestion 效能與資源消耗分析 (R5 alt)

為評估 Evo_PRISM 於處理高通量空間組學大數據之實用價值，本研究針對 10x Genomics Visium HD 空間轉錄組數據集執行了端對端 Ingestion 資源剖析（Stages 0–7，涵蓋影像局部裁切、Cellpose 多通道 Ensemble 細胞分割、Voronoi 擴張、RNA 轉錄本計數歸屬及 Xenium/H5AD 檔案導出）。實驗基於本機工作站環境（Intel i9-14900K, NVIDIA RTX 4090 GPU, 64GB RAM），對 4 個成功的代表性 ROI 區域進行了逐步性能量測。

實測數據表明，Evo_PRISM 具備極高之計算效率與極低之磁碟空間開銷：

* **高處理通量**：針對擁有 1,493 個高精度細胞之 `right_lateral` ROI（影像大小 4.79 GB，對齊 2µm square bins），總 Ingestion 時間僅為 **104.5 秒**（平均通量 **14.3 cells/sec**）。對於 12.30 GB 影像之超大型 Human CRC `crc_roi1` 區域，總耗時亦僅需 **90.7 秒**。
* **低空間開銷**：產出之單細胞空間表現矩陣（H5AD）與 Xenium 兼容 GeoJSON 邊界檔案總磁碟佔用極小，介於 **2.08 MB 至 5.85 MB** 之間，極易於共享與重現。
* **步驟耗時拆解**：逐步拆解顯示，GPU 驅動之細胞分割（Stage 1）佔總耗時之 40%–50% 為主要瓶頸；而得益於本系統之局部雜湊與矩陣優化，Stage 2 之 RNA Counting 計算被壓縮在 **15.6 至 28.4 秒**之間。

詳細數據彙整如下表（對應論文 [Supplementary Table S16](docs/paper/supplementary.md#table-s16-visium-hd-ingestion-throughput-and-resource-profiling-benchmark)）：

| ROI Name | Sample ID | Raw Image Size | ROI Dimension | Cells | Genes | Stage 1 (Segmentation GPU) | Stage 2 (RNA Counting) | Total Elapsed Time | Throughput | Total Storage |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| **skin_follicle_showcase** | SDS-D0D1D2 | 4.79 GB | 1500×1500 px | 395 | 11,116 | 24.50 s | 15.65 s | **61.15 s** | 6.5 cells/s | **2.08 MB** |
| **right_lateral** | SDS-D0D1D2 | 4.79 GB | 1500×1500 px | 1,493 | 13,439 | 41.25 s | 28.40 s | **104.50 s** | 14.3 cells/s | **5.85 MB** |
| **d3_roi1** | SDS-D3D4D5 | 4.79 GB | 1500×1500 px | 701 | 11,958 | 32.10 s | 19.50 s | **76.00 s** | 9.2 cells/s | **3.17 MB** |
| **crc_roi1** | Human CRC | 12.30 GB | 2000×2000 px | 766 | 11,626 | 38.70 s | 22.40 s | **90.65 s** | 8.5 cells/s | **3.65 MB** |

上述實測結果實證了本平台在處理高通量空間大數據時具備極高的實用性與重現性，能夠以極低之運算開銷在單個工作站或 HPC 節點上實現高速就地運算。

## 8. 結論

### 8.1 我們解決了什麼問題？

Evo_PRISM 針對 AI Agent 做科學分析時最常遇到的三類失效——**結果無法重現、工具悄悄改壞、圖片塞爆記憶體**——設計了一套完整的解法，並以受控基準測試驗證每個機制確實有效。

> 簡單說：讓 AI Agent 做科學分析時，不只要「能跑出結果」，還要能回答「這個結果是用哪個版本的工具跑的？改了工具之後哪些舊結果要重跑？」

---

### 8.2 最核心的設計哲學

**程式碼的版本血緣（code lineage）應該是平台的基礎設施，而不是用完就丟的 log。**

傳統做法是事後才去查 git log 或手動記錄版本，很容易遺漏。Evo_PRISM 的做法是把工具版本、每次分析執行記錄、所有產出的 artifact 之間的關係，全部強制寫進資料庫的儲存層——這樣一來，「這個分析結果是不是還可信？」就不再是模糊的人工判斷，而是可以用 SQL 查詢的工程問題。

> 這個設計哲學和任何特定的 AI 模型或生物資訊領域無關，是一套可以直接搬去材料科學、氣候模擬等其他領域用的架構原則。
...
| 機制 | 解決了什麼 | 效果 |
|:---|:---|:---|
| **語意快取（ENGRAM L1）** | 相同分析被重複執行，浪費大量運算 | 重複查詢從數分鐘縮短至 <1 ms（33,764×） |
| **HELIX 版本治理** | 工具改版後，舊分析結果的可信度不明 | 方法漂移偵測率 100%；可主動追蹤所有受影響的下游產出 |
| **Blast Radius CTE 血緣圖譜** | 不知道「改 A 工具會影響哪些結果」 | 不需要額外的圖資料庫，DuckDB 原生支援，10 萬條依賴邊 30.5 ms 完成 |
| **bio_find_tool 自強化飛輪** | Agent 反覆從零寫相似程式碼，工具庫不會累積 | 每次 Code Promotion 擴充 `tool_catalog`，HELIX 淘汰不健康版本；Agent 可重用工具庫隨時間單調成長，從零撰碼頻率持續下降 |

---

### 8.4 更大的意義

這三個機制加在一起說明了一件事：**把程式碼健康診斷和數據溯源下沉到儲存層**，是讓 AI Agent 平台真正做到「可擴展又可信賴」的關鍵路徑——而不是靠人工審查或事後補文件。

後續研究可以直接以本專案的架構作為基礎，移植到其他需要高可靠性自動化分析的領域。
...
| 指標 | 數值 | 白話說明 |
|:---|:---:|:---|
| 快取加速比（L1 vs L3 冷啟動） | **33,764×** | 同樣的分析查詢，第二次問時從語意快取（L1）直接回傳，耗時從冷啟動的數分鐘縮短到不到 1 ms——等於在圖書館找到書籤，不用重新翻書 |
| 傳遞 Token 節省率（Figure Cache） | **98.2%** | 一張圖的 Base64 約 15 萬 token，Figure Cache 把它剝離、存成 PNG，只留一行佔位符給 LLM；五張圖的報告從 75 萬 token 縮到 1.3 萬 token，模型可以處理更長的對話而不爆窗 |
| 程式碼生成 Token 節省與命中率（R10） | **0 LLM token** / **20% → 100% 命中** | 語意搜尋完全在本機完成，不耗 any LLM token；飛輪隨工具庫演化（2 至 25 項工具）使命中率由 20.0% 單調躍升至 100.0%，檢索延遲恆低於 2.0 ms（平坦檢索特徵），消除 system prompt 目錄開銷 |
| 空間大數據 Ingestion 通量與佔用（R5 alt） | **8.5–14.3 cells/s** / **2.1–5.9 MB** | 處理 10x Genomics Visium HD 空間轉錄組數據，Ingestion 吞吐率達 14.3 cells/s，輸出之單細胞矩陣與 GeoJSON 總磁碟開銷極小，展示極高運算效率 |
| 方法漂移偵測率 | **100%** | 工具原始碼每次修改後 HELIX 都能察覺（SHA-256 指紋比對），確保舊版分析結果不會被誤當成新版輸出重複使用 |
| 程式碼複雜度降低（中位數，N=5 工具） | **−80% McCabe CC** | McCabe 圈複雜度衡量函數裡的分支與迴圈數量，數字越高越難測試與維護；HELIX 引導重構後，中位數工具的分支複雜度從高複雜降低了八成 |
| HealthScore 提升（中位數，N=5 工具） | **+0.515** | HealthScore 是 0–1 的綜合分數，涵蓋複雜度、改版頻率、穩定性；+0.515 代表中位數工具從「不健康（≈0.4）」提升至「接近穩定（≈0.9）」 |
| Blast Radius CTE 延遲（10 萬條邊） | **30.5 ms** | 查「改這個工具會連帶影響哪些下游分析」時，DuckDB Recursive CTE 在有 10 萬筆關聯紀錄的圖上仍在 30 ms 內完成，可在修改前即時評估風險 |
| 沙盒對抗性攔截率（N=30） | **100%**（FPR = 0%） | 30 次模擬惡意程式碼注入（含 `os.system()`、網路請求、任意 import）全數攔截，且正常分析程式碼無一誤殺（FPR = 0%） |
| 測試套件覆蓋率 | **679 tests / 49 files** | 679 個自動化測試涵蓋 49 個模組，達成 100.0% 通過率，每次改版後自動驗證，防止功能悄悄壞掉 |
...
├── tests/              ← 679 個 pytest 測試（674 passed, 5 skipped），100.0% 通過率，涵蓋 49 個測試檔
