...
| 程式碼生成 Token 節省（bio_find_tool） | **0 LLM token**（搜尋本身） | 語意搜尋完全在本機完成（bge-m3 embedding + DuckDB HNSW），不耗任何 LLM token；catalog 命中時 Agent 直接呼叫既有函數，每次節省重新生成整段分析程式碼的數百至千餘 token；飛輪累積後命中率持續提升，節省效果隨時間增強 |
