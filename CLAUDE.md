# Evo_PRISM (Evo-PRISM) 專案憲法 — 執行期智慧與語意記憶

## 1. 專案定位

**Evo_PRISM (Evo-PRISM: Evolutionary Platform for Runtime Intelligence & Semantic Memory)** 是一個通用自進化執行期智慧與語意記憶平台，以 AI Agent + MCP 為核心。
...
├── .env.example            ← 環境變數範本
├── bio_memory.duckdb       ← 主 DuckDB（sample_registry + analysis_history，本機資料庫）
│
...
│
├── proteome_data/          ← Proteomics 數據（L2 ready，本地數據目錄）
...
│
├── silver/                 ← L2：Parquet 特徵存儲（由腳本寫入，本地數據目錄）
├── gold/                   ← L1：語意快取（由分析函數寫入，本地數據目錄）
│
├── docs/                   ← 專案操作文件（集中管理所有設計規劃與進度封存）
│   ├── images/             ← 架構圖（三層架構.png / HELIX / ENGRAM）
│   ├── guides/             ← 操作指南（DATA_INTEGRATION / MCP_JSON / L3_INGEST 等）
│   ├── plans/              ← 設計計畫（plan_zh.md / plan.md / IMPLEMENTATION_PLAN 等）
│   ├── launchd/            ← macOS launchd plist 範本
│   ├── logs/               ← 開發日誌（PROGRESS.md / execution_trace.md）
│   └── (架構評估與審查報告)
...
```bash
cp docs/launchd/launchd_embedding_server.plist.example \
...
# 自動啟動
cp docs/launchd/launchd_multimodal_server.plist.example \
...
```bash
# 手動備份（launchd 每日 02:00 自動跑，見 docs/launchd/launchd_backup.plist.example）
...
詳細決策流程見 [docs/guides/DATA_INTEGRATION_GUIDE.md](docs/guides/DATA_INTEGRATION_GUIDE.md)。
...
| 文件 | 內容 |
|------|------|
| [docs/logs/PROGRESS.md](docs/logs/PROGRESS.md) | 當前進度、完成里程碑、待辦事項 |
| [docs/plans/IMPLEMENTATION_PLAN.md](docs/plans/IMPLEMENTATION_PLAN.md) | 當前 Phase 執行計畫（含驗收標準） |
| [docs/logs/execution_trace.md](docs/logs/execution_trace.md) | Phase 執行紀錄（結果與 commit hash） |
| [docs/plans/plan_zh.md](docs/plans/plan_zh.md) | 完整七階段系統設計（中文） |
| [docs/plans/plan.md](docs/plans/plan.md) | 完整系統設計（英文） |
| [docs/guides/L3_DATA_INGEST_GUIDE.md](docs/guides/L3_DATA_INGEST_GUIDE.md) | 新增樣本到 L3 的操作指南 |
| [docs/guides/TEST_DATABASE_INDEX.md](docs/guides/TEST_DATABASE_INDEX.md) | 測試資料庫總覽（數據位置、大小） |
| [docs/guides/DATA_INTEGRATION_GUIDE.md](docs/guides/DATA_INTEGRATION_GUIDE.md) | 跨專案數據與程式碼整合決策指南 |
| [docs/guides/MCP_JSON_SETUP.md](docs/guides/MCP_JSON_SETUP.md) | `.mcp.json` 設定指南（env vars、安全建議、路徑處理） |
| [docs/guides/MCP_HTTP_GUIDE.md](docs/guides/MCP_HTTP_GUIDE.md) | MCP HTTP transport 使用說明（curl 範例、Accept header、工具表） |
| [docs/launchd/launchd_backup.plist.example](docs/launchd/launchd_backup.plist.example) | macOS 每日備份排程範本 |
