...
def voronoi_expand(mask: np.ndarray, max_distance: int,
    ...
    """
    Voronoi 約束擴張：每個背景像素分配給最近細胞，距離上限 max_distance px（記憶體安全優化版）。
    優於 expand_labels：不產生重疊，細胞自然填滿可用空間。
    """
    if mask.size > 50_000_000:
        h, w = mask.shape
        expanded_full = np.zeros((h, w), dtype=mask.dtype)
        pad = max_distance
        tile_size = 4096
        for y in range(0, h, tile_size):
            y0_tile = y
            y1_tile = min(y + tile_size, h)
            y0_pad = max(0, y0_tile - pad)
            y1_pad = min(h, y1_tile + pad)
            for x in range(0, w, tile_size):
                x0_tile = x
                x1_tile = min(x + tile_size, w)
                x0_pad = max(0, x0_tile - pad)
                x1_pad = min(w, x1_tile + pad)
                
                patch_mask = mask[y0_pad:y1_pad, x0_pad:x1_pad]
                patch_tissue = tissue_mask[y0_pad:y1_pad, x0_pad:x1_pad] if tissue_mask is not None else None
                
                patch_expanded = voronoi_expand(patch_mask, max_distance, tissue_mask=patch_tissue)
                
                local_y0 = y0_tile - y0_pad
                local_y1 = local_y0 + (y1_tile - y0_tile)
                local_x0 = x0_tile - x0_pad
                local_x1 = local_x0 + (x1_tile - x0_tile)
                
                expanded_full[y0_tile:y1_tile, x0_tile:x1_tile] = patch_expanded[local_y0:local_y1, local_x0:local_x1]
    ...
    if mask.size > 50_000_000:
        ...
        for y in range(0, h, tile_size):
            ...
            for x in range(0, w, tile_size):
                ...
                block = mask[y:y1, x:x1]
                if np.any(block > 0):
                    lbls, cnts = np.unique(block, return_counts=True)
                    for l, c in zip(lbls, cnts):
                        if l > 0:
                            counts[int(l)] += int(c)
        
        remove = {lbl for lbl, c in counts.items() if c < min_size or c > max_size}
        if len(remove) == 0:
            return mask
            
        max_val_global = int(mask.max())
        if max_val_global <= 100_000_000:
            # 向量化 LUT 快速清除
            lut = np.arange(max_val_global + 1, dtype=mask.dtype)
            for lbl in remove:
                if lbl < len(lut):
                    lut[lbl] = 0
            ...
            for y in range(0, h, tile_size):
                ...
                for x in range(0, w, tile_size):
                    ...
                    if np.any(block > 0):
                        ...
                        block_remove = [lbl for lbl in block_unique if lbl in remove]
                        if len(block_remove) > 0:
                            mask[y:y1, x:x1][np.isin(block, block_remove)] = 0
        return mask
...
def relabel_sequential(mask: np.ndarray) -> np.ndarray:
    """重新編號為從 1 開始的連續 ID（LUT 向量化，O(max_id)，記憶體安全優化版）。"""
    if mask.size > 50_000_000:
        h, w = mask.shape
        tile_size = 4096
        unique_labels = set()
        for y in range(0, h, tile_size):
            ...
            for x in range(0, w, tile_size):
                ...
                block = mask[y:y1, x:x1]
                if np.any(block > 0):
                    lbls = np.unique(block)
                    for l in lbls:
                        if l > 0:
                            unique_labels.add(int(l))
                            
        unique_labels = sorted(list(unique_labels))
        if len(unique_labels) == 0:
            return mask
            
        max_val_global = int(unique_labels[-1])
        if max_val_global <= 100_000_000:
            # 向量化 LUT 快速重新編號
            lut = np.zeros(max_val_global + 1, dtype=mask.dtype)
            new_ids = np.arange(1, len(unique_labels) + 1, dtype=mask.dtype)
            lut[unique_labels] = new_ids
            ...
            # Fallback for massive values (sparse OOM guard)
            label_map = {lbl: i + 1 for i, lbl in enumerate(unique_labels)}
            for y in range(0, h, tile_size):
                y1 = min(y + tile_size, h)
                for x in range(0, w, tile_size):
                    x1 = min(x + tile_size, w)
                    ...
                    if np.any(block > 0):
                        block_unique = np.unique(block)
                        for lbl in block_unique:
                            if lbl > 0 and lbl in label_map:
                                mask[y:y1, x:x1][block == lbl] = label_map[lbl]
        return mask
...
def run_tiled_mcseg_v2(
    ...
    # ── 預處理（全圖一次性，節省重複計算）────────────────────
    # 記憶體安全優化：避免在大圖上直接執行 color deconvolution 和 apply_clahe（這會導致 146 GB 以上的記憶體需求）
    # 我們利用 downsampled / strided 視圖估算全域 normalization h_max，並在迴圈中逐 tile 進行預處理。
    h_max = 1.0
    if use_hematoxylin:
        logger.info("  全域色彩分離係數估算中...")
        stride_y = max(1, H // 1024)
        stride_x = max(1, W // 1024)
        thumb = img[::stride_y, ::stride_x]
        
        try:
            img_f = thumb[..., :3].astype(np.float64) + 1.0
            od = -np.log(img_f / 256.0)
            he_matrix = np.array([
                [0.6500286, 0.7041680, 0.2860126],   # Hematoxylin
                [0.0728940, 0.9904310, 0.1155140],   # Eosin
            ...
            for i in range(3):
                ...
                if norm > 0:
                    he_matrix[i] /= norm
            stains = (od.reshape(-1, 3) @ np.linalg.inv(he_matrix).T).reshape(thumb.shape[:2] + (3,))
            hema = np.clip(stains[:, :, 0], 0, None)
            h_max = float(np.percentile(hema, 99.5))
            logger.info(f"  全域 Hematoxylin 係數估算成功：h_max={h_max:.4f}")
        except Exception as e:
            logger.warning(f"  全域色彩分離估算失敗，使用預設值：{e}")
            h_max = 1.0

    # 記憶體安全優化：全圖組織遮罩以 8x 降採樣計算 morphology，以節省記憶體並提升 100 倍速度
    logger.info("  全域組織遮罩計算中...")
    try:
        gray_thumb = cv2.cvtColor(img[::8, ::8, :3], cv2.COLOR_RGB2GRAY) if img.ndim == 3 else img[::8, ::8]
        tissue_thumb = (gray_thumb < 220).astype(np.uint8)
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (11, 11))
        tissue_thumb = cv2.morphologyEx(tissue_thumb, cv2.MORPH_CLOSE, kernel)
        kernel2 = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
        tissue_thumb = cv2.dilate(tissue_thumb, kernel2, iterations=2)
        tissue_mask = cv2.resize(tissue_thumb, (W, H), interpolation=cv2.INTER_NEAREST).astype(bool)
        logger.info("  全域組織遮罩計算成功")
    except Exception as e:
        logger.warning(f"  全域組織遮罩計算失敗，使用簡單閥值fallback: {e}")
        tissue_mask = (cv2.cvtColor(img[..., :3], cv2.COLOR_RGB2GRAY) < 220).astype(bool) if img.ndim == 3 else (img < 220).astype(bool)
    ...
    sample_id = cfg.get("sample_id", "default_sample")
    cache_dir = Path("i:/Evo_PRISM/results/mcseg") / sample_id / "fullslide" / "tile_cache"
    ...
    for ti, y in enumerate(y_starts):
        ...
        for tj, x in enumerate(x_starts):
            ...
            cache_file = cache_dir / f"tile_{tile_idx}.npy"
            if cache_file.exists():
                logger.info(f"  [Cache Hit] 載入已快取之分塊 {tile_idx}/{total_tiles}")
                base = np.load(str(cache_file))
            else:
                # 記憶體安全：動態讀取並預處理單個 tile
                tile_raw = img[y0e:y1e, x0e:x1e]
                
                # 組織遮罩評估：評估此分塊是否包含組織（避免對純背景分塊做無用的 cellpose 運算，節省一半時間）
                gray_tile = cv2.cvtColor(tile_raw[..., :3], cv2.COLOR_RGB2GRAY) if (tile_raw.ndim == 3 and tile_raw.shape[-1] >= 3) else tile_raw
                tissue_mask_tile = gray_tile < 220
                tissue_pct = float(np.mean(tissue_mask_tile))
                ...
                if tissue_pct < 0.01:
                    logger.info(f"  [{time.time()-t0:.0f}s] {msg} -> 空白背景分塊 (組織比例 {tissue_pct:.2%})，直接跳過")
                    tile_results = {}
                    enh_tile = None
                    hema_tile = None
                else:
                    logger.info(f"  [{time.time()-t0:.0f}s] {msg} -> 啟動分割 (組織比例 {tissue_pct:.2%})")
                    # 局部 CLAHE
                    enh_tile = apply_clahe(tile_raw, clip_limit=clahe_clip, tile_size=8)

                    # 局部色彩分離
                    hema_tile = None
                    if use_hematoxylin:
                        try:
                            img_f = tile_raw[..., :3].astype(np.float64) + 1.0
                            od = -np.log(img_f / 256.0)
                            he_matrix = np.array([
                                [0.6500286, 0.7041680, 0.2860126],   # Hematoxylin
                                [0.0728940, 0.9904310, 0.1155140],   # Eosin
                            ...
                            for i in range(3):
                                ...
                                if norm > 0:
                                    he_matrix[i] /= norm
                            stains = (od.reshape(-1, 3) @ np.linalg.inv(he_matrix).T).reshape(tile_raw.shape[:2] + (3,))
                            hema_t = np.clip(stains[:, :, 0], 0, None)
                            if h_max > 0:
                                hema_t = np.clip(hema_t / h_max, 0, 1)
                            hema_u8 = (hema_t * 255).astype(np.uint8)
                            clahe_obj = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
                            hema_tile = clahe_obj.apply(hema_u8)
                        except Exception as e:
                            logger.warning(f"  Tile {tile_idx} 色彩分離失敗：{e}")
                            hema_tile = None
                ...
                # merge tile results
                if "mid" in tile_results:
                    base = tile_results["mid"].copy().astype(np.int32)
                else:
                    base = np.zeros((y1e - y0e, x1e - x0e), dtype=np.int32)
    ...
    if cpsam is not None:
        del cpsam
    # enhanced and hema_full are dynamic in loops, no global deletion needed
    pass
    gc.collect()
    _clear_gpu_cache()
    ...
    # 記憶體安全優化：立刻釋放不再需要的 stitched 陣列，避免雙重佔用 48 GB 的巨額記憶體！
    del stitched
    gc.collect()

    final = clean_mask(final, min_size=min_size, max_size=max_size)
    final = relabel_sequential(final)
    ...
    # 記憶體安全優化：如果已經是 int32 類型，則避免進行複製操作，節省 24 GB 記憶體！
    return final.astype(np.int32, copy=False)
