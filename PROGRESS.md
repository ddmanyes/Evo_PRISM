...
### P1-E：測試環境 pytest assertion rewriting workaround（2026-05-19 部分完成）
...
- [ ] **`tests/test_tool_visualizer.py` 同類問題 2 fail**（`TestComputeLoc::test_returns_positive_int` / `test_branchy_has_more_loc_than_simple`）：`compute_loc()` 直接呼叫無 fixture 攔截點。預定建立 `tests/_visualizer_stubs.py`（普通 module，不被 pytest rewrite），但本 session 暫停未做
- **不動 production code**：所有 workaround 限定在 fixture lifecycle / test-only module
- **後續驗收**：跑 `pytest tests/` 全綠（目前 284 passed / 2 failed / 5 skipped）
