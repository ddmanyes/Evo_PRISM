...
> [!IMPORTANT]
> **✉️ 請聯絡作者取得下載連結 / Contact Author for Download Link**
> 
> 由於測試數據體積龐大且包含實驗室內部敏感數據，並未上傳至 Git 儲存庫。如需取得測試資料壓縮包，請聯絡作者：
> **詹麒儒 (Chan Chi Ru) — u9013039@gmail.com**

### 隨 repo 附帶的 Demo 數據 / Demo Data Included in Repo

`tests/fixtures/bulk_rna/` 內含可直接使用的小型示範數據集：

*`tests/fixtures/bulk_rna/` contains a small ready-to-use demo dataset:*

| 檔案 | 大小 | 說明 |
|------|------|------|
| `deseq2_counts_top1000.csv` | ~400KB | Bulk RNA-seq count matrix（1000 個最高變異基因 × 84 樣本，DESeq2 格式）|
| `deseq2_coldata.tsv` | 2KB | 樣本 metadata（group 欄位，DESeq2 必需）|

可直接用於 `bio_run_deg`、`bio_run_enrichment`、`bio_run_heatmaps` 工具 demo。

*Can be used directly to demo `bio_run_deg`, `bio_run_enrichment`, and `bio_run_heatmaps` tools.*

### 完整測試資料 / Full Test Dataset

完整測試資料（L2 Parquet + DuckDB + 空間轉錄體數據）體積龐大（~39 GB），請聯絡作者取得：

*The full test dataset (L2 Parquet + DuckDB + spatial transcriptomics) is ~39 GB. Contact the author:*
