...
### 完整數據集（空間轉錄體 + 完整 DuckDB）/ Full Dataset

完整數據集（CRC Visium HD 8µm L2 Parquet + 已初始化的 `bio_memory.duckdb` Schema v21）體積約 **39 GB**，為實驗室內部數據，未上傳至 git。

*The full dataset (~39 GB, CRC Visium HD L2 Parquet + initialized `bio_memory.duckdb` v21) contains proprietary lab data and is not in this repo.*

完整資料包含 / Full archive includes:
- `bio_memory.duckdb` — 已初始化主資料庫（Schema v21，含範例 sample 登記）
- `silver/spatial_counts_crc_official_v4_8um/` — CRC Visium HD 8µm L2 Parquet（416 MB，215M nonzero）

> [!NOTE]
> **✉️ 如需完整測試數據，請聯絡作者 / Full dataset available on request**
>
