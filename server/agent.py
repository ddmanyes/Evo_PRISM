...
4. **bio_run_spatial_eda**：L2 Parquet 讀取，生成空間轉錄體 EDA 報告（含摘要寫入 L1 快取）。
5. **bio_run_bulk_eda**：Bulk RNA-seq EDA（QC + top genes + PCA），需先執行 pipeline 腳本產生計數矩陣。
6. **bio_execute_code**：非標準分析，動態生成並沙盒執行 Python 程式碼。
