...
呼叫 `bio_tool_health` 的時機：
- 使用者詢問「工具是否穩定」、「工具庫有無臃腫」→ `action=report`
- 發現某工具 revision_count 高且有明顯重複修改模式 → `action=diagnose`（主動寫入診斷）
- 使用者要求清理舊版本 → `action=prune`（只刪無引用的 deprecated，絕不刪有分析紀錄引用者）

**穩定化迭代原則**：
- 熱區工具（revision ≥ 3）優先診斷，stability_note 記錄「為何頻繁修改」及「建議抽象化方向」
- 穩定工具（revision < 3）可積極 prune，節省空間
- 不穩定工具保留更多歷史（預設保留 10 個版本），供未來追溯
...
BIO_TOOLS = [
    ...
    },
    {
        ...
        "description": (
            "工具庫健康報告。回傳：active/deprecated 工具數量、熱區工具（revision_count≥3）、"
            "可安全清理的 deprecated 紀錄數、過期分析結果數，及改善建議。"
            "支援三個 action：\n"
            "  'report'   — 查詢健康狀態（預設）\n"
            "  'diagnose' — 對某工具寫入 stability_note（需提供 tool_name + note）\n"
            "  'prune'    — 清理未被引用的 deprecated 紀錄（需提供 tool_name）"
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "action": {
                    "type": "string",
                    "enum": ["report", "diagnose", "prune"],
                    "description": "操作類型",
                },
                "tool_name": {
                    "type": "string",
                    "description": "diagnose/prune 時必填，工具邏輯名稱",
                },
                "note": {
                    "type": "string",
                    "description": "diagnose 時必填，說明此工具為何頻繁變動及建議穩定化方向",
                },
            },
            "required": ["action"],
        },
...
def _exec_bio_tool_health(args: dict) -> str:
    ...
    if action == "report":
        ...
        if report["hot_zones"]:
            ...
            for t in report["hot_zones"]:
                ...
                lines.append(f"  {t['tool_name']}  revision={t['revision_count']}  診斷：{note}")
                for entry in t["change_log"][:3]:
                    reason = entry["reason"] or "—"
                    lines.append(f"    [{entry['revision']}] {entry['old_hash'] or 'init'} → {entry['new_hash']}  {entry['changed_at'][:16]}  {reason}")
        else:
        ...
        if report["prune_candidates"]:
            ...
            for name, n in report["prune_candidates"].items():
                lines.append(f"  {name}: {n} 筆（呼叫 bio_tool_health action=prune 執行）")
        ...
        return (
            ...
            "此診斷將在下次 bio_tool_health report 中顯示，並作為穩定化迭代的參考依據。"
        )
    ...
    return f"[Error] 未知 action: {action!r}，請使用 report / diagnose / prune。"
