> **Source PDF:** `references/pdfs/llmlingua_emnlp2023.pdf`
...
<!-- Page 1 -->
 ...
 words from a fixed vocabulary. In this paper, we formulate text generation as2023 progressively copying text segments (e.g., words or phrases) from an existing text
 collection. We compute the contextualized representations of meaningful text
 segments and index them using efficient vector search toolkits. The task of textJul
 generation is then decomposed into a series of copy-and-paste operations: at each
13 timefrom step,a standalonewe seek suitablevocabulary.text spansExperimentsfrom theontextthecollectionstandard ratherlanguagethanmodelingselecting
 ...
 adaptation by simply switching to domain-specific text collection without extra[cs.CL] training. Finally, we observe that our approach attains additional performance gains
 ...
 from existing text collections. The text segments can be of variable lengths, including single wordsarXiv:2307.06962v1 and multi-word phrases. For clarity, we will use the term “phrase” to refer to any contiguous text
...
manner pθ(x0:n) = Qni=1 p(xi|x<i). In this case, text generation is reduced to the task of repeatedly
...
document Di, a phrase k = Dis:e of length e −s + 1 can be extracted, where s and e mark the start
...
Published as a conference paper at ICLR 2023
 ...
 �������� ������ … in theaters on … and was extremely … the film premiered
 October 22, 2021 in well-received by critics at the 78�ℎ … BERT model the United States … and audiences … … … + … September 3, 2021.

 in States and audiences the .
 Source Text Token Embeddings
 Collection
 Copy
 Prefix Encoder Maximum Inner Product Search
 […][…] […][…]
...
Ethical Consideration The text generated by COG contains text segments copied from other
...
Prefix Encoder The prefix encoder is responsible for encoding the prefix x<i into a vector representation for the next-phrase prediction. We treat the prefix as a sequence of tokens (previously predicted
...
Phrase Encoder Given a set of source documents {D1, ..., Dn}, the phrase encoder computes the
...
dimension of token representations. Then, we apply two MLPs models, MLPstart and MLPend, to
 2 , respectively:convert D into start and end token representations Dstart, Dend ∈Rm× d
...
Context-Independent Token Embeddings Although COG can copy phrases from other documents,
...
InfoNCE loss with in-batch negatives (Karpukhin et al., 2020):
 ...
 n
 exp(qk · pk) X log Lp = −1
 P n p∈Pk exp(qk · pp) + Pw∈V exp(qk · vw) k=1
...
token-level autoregressive loss.
 ...
 m
 exp(qi, vDi) X log Lt = −1
 m Pw∈V exp(qi, vw) i=1
...
et al., 2020; Su et al., 2022) and report the results on the following evaluation metrics:
 ...
 can be formulated as: 100 × (1.0 −|unique|total n−gram(x)|n−gram(x)| ). Higher Rep-n denotes the severe
 degeneration problem in generations.

 • Diversity (Welleck et al., 2020) measures the diversity of the generations, which is formu-
 lated as Π4n=2(1 −Rep−n100 )). Generations that have higher Diversity scores usually are more
 informative.
...
language modeling (Khandelwal et al., 2020; Dai et al., 2019; Su et al., 2022).
 ...
 greedy 19.87 43.56 38.55 35.5 22.37 1.32
 Transformer
 nucleus 23.43 5.10 1.33 0.50 93.22 1.48
 greedy 19.92 43.79 38.76 35.69 22.13 10.36
 kNN-LM
 nucleus 22.50 3.33 0.69 0.21 95.8 10.42
 greedy 21.19 44.65 39.63 36.6 21.19 4.39
 RETRO
 nucleus 22.86 6.21 1.93 0.86 91.19 4.51
 greedy 26.01 28.14 23.80 21.40 43.03 1.29
 COG
...
Results Table 1 shows the performance comparison between the baselines and our proposed COG
...
Published as a conference paper at ICLR 2023
 ...
 In 2000 Boulter had a guest- The Man Trap ”. He is also involved in an ongoing
 starring role on the television
 series The Bill ; he portrayed "
 Scott Parry " in the episode, " Document Document Document

 … like Metallica . He is also … However , Jerry Hardin , who
 Input Prefix The first regular episode ( " The Man influenced by orchestral and was involved in another morph to Trap " ) of Star Trek: The Original … classical composer … his character Deep Throat, …

 science fiction drama series called the Magic Roundabout television series The Man Trap ”. He is also involved in
 an ongoing science fiction drama series
 called the Magic Roundabout television Document Document Document series …
 … she became the youngest Eric Thompson who narrated The Magic … of Thor with the high-tech actress in a drama series Roundabout television series , was born science fiction in Iron Man … when she starred … in a house on Jermyn Street Generated Continuation
...
Inference Speed Furthermore, we Method Uni-gram 2-gram 3-gram 4-gram 5-gram 6-gram
also compare the average time cost of Greedy 0.583 0.195 0.121 0.056 0.029 0.017
different methods for completing the
 Nucleus 0.434 0.219 0.181 0.09 0.048 0.028
generation on the test set. Since the
phrase representations in COG are pre- Table 2: The statistics on the length of the copied phrases
computed offline, its encoding time (on the test set of WikiText-103).
...
Human Evaluation To ensure the reliability of Comparison Better No Prefer. Worse
our evaluations, we also run human evaluation with
 COG vs.
three native-speaker graders from a third-party grad- 48% 24% 28%
 Transformering platform. Specifically, we randomly select 100
test prompts. For each test prompt, the annotators
 Table 3: Human evaluation on the WikiText-are given two continuations, in random order, which
 103 corpus.are generated by COG and Transformer respectively.
...
Case Study For a better understanding of the performance of COG, we present an example of the
...
Results As shown in Table 4, it can be ob- Model Decoding MAUVE ↑Diversity ↑
served that COG even outperforms the Trans- greedy 20.32 70.66
former model further fine-tuned on the Law- Transformer w/o FT nucleus 25.21 93.88
MT corpus (Transformer w/ FT). Specifically, Transformer w/ FT greedy 23.00 80.52
COG outperforms Transformer w/ FT by 2.93% nucleus 26.85 90.14
MAUVE score. The results indicate that COG kNN-LM greedy 23.31 19.85
allows a single model to be specialized in dif- nucleus 24.75 94.60
 greedy 18.70 71.14
ferent domains, by simply switching the source RETRO nucleus 20.35 94.81
text collection. Although kNN-LM brings in greedy 21.31 84.32
higher Diversity scores, COG surpasses it by COG nucleus 28.14 92.56
3.39% MAUVE score, which shows COG has
higher generation quality in general. Table 4: The automatic evaluation on Law-MT.

Human Evaluation We also conduct the hu- Comparison Better No Prefer. Worse
man evaluation on the Law-MT corpus, which
 COG vs.
has a similar setup to that in (§5.1). Table 5 52% 12% 36%
 Transformer w/ FTshows that most of COG’s generations are better than a strong Transformer baseline. This
 Table 5: Human evaluation on Law-MT.observation demonstrates that COG can even
...
Results The experimental results are shown in Table 6. COG with En-Wiki memory surpasses
...
Effects of Index Size To further investigate how the size of the phrase index affects the generation
...
Published as a conference paper at ICLR 2023
 ...
 27.0
 greedy 19.87 22.37 CoG
 Transformer w/o FT 26.5
 nucleus 23.43 93.22
 greedy 20.21 19.62 26.0
 Transformer w/ FT nucleus 21.31 92.92 score 25.5
 greedy 23.21 20.33 25.0
 kNN-LM nucleus 23.39 96.37 MAUVE 24.5
 greedy 19.75 21.15
 RETRO 24.0
 nucleus 22.87 91.09 Transformer w/o FT
 23.5
 greedy 24.68 40.45
 COG 0.0 0.001 0.003 0.01 0.03 0.1 0.3 1.0
 nucleus 26.97 90.00 The proportion of enlarged En-Wiki Documents

Table 6: The automatic evaluation on the test set Figure 3: Generation quality of COG with differof WikiText-103, the memory is built on the train ent sizes of the phrase index. For each proportion
set of En-Wiki. Transformer w/ FT and Trans- (point in the X-axis), we sample 10 times and
former w/o FT denote the Transformer baseline record the averaged MAUVE score. A proporwith and without further fine-tuning on the train tion of 0.0 indicates that only documents from
set of En-Wiki, respectively. WikiText-103 are used.

6 RELATED WORK

Dense Retrieval The dense retrieval technique (Karpukhin et al., 2020) has been widely used in
...
The most closely related work to our study is DensePhrase (Lee et al., 2021). DensePhrase reformulates the question-answering task as a phrase retrieval problem, where phrases are directly retrieved
...
Retrieval-Augmented Text Generation (RAG) Retrieval-augmented text generation has gained
...
We calculate the perplexity of the generated texts un- Models Perplexity
der a large pre-trained language model (GPT2-Large). greedy nucleus
 Transformer 3.26 37.11As shown in Table 8, it can be found texts gener-
 kNN-LM 3.48 78.01
ated by greedy search can achieve very low perplex-
 RETRO 3.27 36.40
ity scores (even much lower than the ground-truth)4. COG 10.41 27.24
This is expected as greedy search targets at likelihood Ground-Truth 18.64
maximization. Sampling-based decoding methods
give much higher perplexity scores. Moreover, it is
 Table 8: The perplexity on the test set ofworth noting that COG achieves the closest perplexity
 WikiText-103.score to ground-truth.
...
Algorithm 1: Phrase Segmentation Algorithm
 Data: Document set: D = {di, {dj}Kj=1}Ni=1, where di denotes the i-th document. K denotes
 ...
 Result: Segmented document set by phrase granularity: D′ = {{(pi,x, (dj, posj))}||di||px=1 }Ni=1,
 ...
 The preprocessed document set can be formulated as D = {{ti,x}||di||tx=1 , {dj}Kj=1}Ni=1, where
...
inaccurate generation results (rivers).
 ...
 Golden Globe Award for Best comer. In 2008 , Nolan was announced as the star of
 Actor. Gleason and Scott were
 each nominated for Best
 Supporting Actor and Scott was Document Document Document
 also nominated as Best New
 While Bart had been the star Input Prefix Inthe2008A &, McCormackE television co-starredminiseries in announcedOn 21 Januaryas the2016,player-managerNolan was of the show during … The Andromeda Strain … of Leyton Orient, …

 Seven Psychopaths . He cast Matthew Morrison as
 Comer. In 2008, Nolan was announced
 as the star of Seven Psychopaths. He
 Document Document Document cast Matthew Morrison as
 After premiering at the Toronto Film … Of the principal cast , He cast her in the role . Festival, Seven Psychopaths ( Farrell’s Ryan said : " Casting She portrayed …
 second movie … Matthew Morrison as … Generated Continuation
...
in blue ) is copied from other documents.
 ...
 In the Philippines, officials with a peak wind gust of 106 km/h ( 75 mph )
 evacuated over 14 @,@ 000
 people. Imbudo was the
 strongest typhoon to strike since Document Document Document Typhoon Zeb five years prior,
 Cyclone Calasanjy caused heavy … at the Wilmington International
 Input Prefix damage in western Madagascar , Airport , while gusts reached 66 … sustained winds of at least with a peak wind gust of 195 120 km / h ( 75 mph ) … mph ( 106 km / h ) at … km/h …

 being measured on Pagan Island . with a peak wind gust of 106 km / h
 ( 75 mph ) being measured on Pagan
 Document Document Island.
 Texas coastal plain led to extremely high … with gusts to 183 km / h ( 114
 rainfall totals being measured in parts of mph ) on Pagan Island . The
 Jefferson … typhoon caused heavy crop … Generated Continuation
...
Published as a conference paper at ICLR 2023
 ...
 The Battle of Defcrenstein ( also Unternehmen Aster isk ), was a battle fought in the Ardennes forest
 known as the Battle of <unk>,
 Battle of <unk> and Battle of
 <unk> ; German : Document Document Document
 … fought on September 7 , 1812 , … in a retreat codenamed The offensive against American
 Input Prefix Operation Aster ( German : was a battle fought in the forces in the Ardennes forest had Napoleonic Wars during the Unternehmen Aster ) … preoccupied Hitler 's mind … French invasion of Russia.

 near the junction of the Rhine and Main rivers on 8 October Unternehmen Asterisk ), was a battle fought in
 the Ardennes forest near the junction of the
 Document Document Document Rhine and Main rivers on 8 October
 By the terms of the Peace of Basel … independence in June 1991 ; … Spafford Farm , was
 ( 22 July 1795 ) , the Prussian army however the declaration came into located near the junction of was to leave the Rhine the Spafford 's Branch … and Main effect on 8 October 1991 … Generated Continuation river valleys …
