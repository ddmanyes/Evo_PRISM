...
同一個 CRC 樣本，A 成員週一跑了 QC，B 成員週三不知道，重跑了一遍——這在實驗室每週發生。SpaceRanger 單次 ~4 小時，浪費的不只是算力，還有等待的時間與對結果的不確定感。

根本原因是四個結構性缺口：

1. **重複運算**：分析結果沒有集中記錄，成員各自重跑而不自知
2. **數據孤島**：結果散落各人電腦，無法統一查詢或跨樣本比較
3. **無分析記憶**：不知道某樣本做過什麼、結果在哪、用了哪個版本的工具
4. **使用門檻**：不熟悉命令列的成員無法自助取得結果，依賴少數人轉手

### 核心主張

**記住曾經做過的分析——相同的問題不必重算，新的分析能與過去比較，累積的分析路徑更能推導出新的方法與洞見。**

每次分析完成後，結果、參數、工具版本、產出檔案全部自動存入資料庫、永久保留。系統提供三個層次的記憶價值：

1. **去重**：相同或語意相近的問題直接從歷史取回，不重新執行、不消耗算力
2. **比較**：新分析完成後可隨時與同一樣本的歷史結果並排比對，累積越多、比較的基準越豐富
3. **推導**：過去的分析路徑（做了什麼、怎麼做、用了哪個版本）構成可查詢的知識脈絡，幫助歸納規律、發現可重用的方法，甚至推導出原本不會主動嘗試的新分析方向

### 實現方式

以 **AI Agent** 為核心，三個層次協同運作：

- **人機介面**：Web UI 或 Telegram 自然語言提問，無需程式能力；支援圖片上傳讓視覺模型直接分析圖表
- **去重閘道**：避免重複計算——問題進來後依序在三個維度攔截：

  - **問題維度**：SQL 精確比對（0 token）→ HNSW 語意搜尋（少量 token），攔截「問過的問題」
  - **程式碼維度**：3B 程式碼重用，攔截「生成過的分析邏輯」，相同邏輯不重新生成
  - 三關都未命中，才真正執行新分析
- **記憶核心**：新分析完成後，三個核心協同保存完整的實驗室知識記憶：

  - `analysis_history` 記錄「做了什麼」——分析類型、參數、狀態、執行時間
  - **ENGRAM**（本系統原創）記錄「產出了什麼、怎麼來的」——每個產出檔案（圖、CSV、報告）的內容、MIME、語意 embedding，以及 provenance hash（輸入資料來源、程式碼版本、執行環境）；提供 RRF 混合搜尋與並排比較介面，讓產出可被查詢、比較與推導
  - **HELIX**（本系統原創）記錄「工具怎麼演化的」——每次修改以 content-hash 版本化，累積修改歷史；從記憶中自動偵測反覆被改動的熱區工具並觸發穩定化迭代，讓程式碼在持續使用中被雕琢成穩定可信的分析工具

  三者透過 `analysis_id` 與 `tool_id` 相互連結——知道做了什麼分析、產出了什麼結果、結果由哪個版本的工具產生，構成可追溯、可推導的完整知識脈絡
