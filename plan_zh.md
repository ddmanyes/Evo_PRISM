...
- **三軌記憶與觀測核心（Triple Memory & Observability Core）**：
  - **ENGRAM（產出記憶）**：記錄「產出了什麼、如何溯源」。持久化 `analysis_artifacts` 並實施大檔案分離（500 KB 閾值 Blob 分離，migration v14）。結合 HNSW 與 RRF（倒排倒數融合，migration 9A-2）混合搜尋，提供可視化 Lightbox 及並排比對。
  - **HELIX（工具演化記憶）**：記錄「分析工具如何被雕琢」。對註冊工具進行 Content-Hash 版本化追蹤，透過 McCabe 循環複雜度與代碼變動率（Relative Code Churn）評估工具健康度。結合 Ebbinghaus 遺忘曲線將過期工具降採樣快照（640x640 PNG），確保工具庫精簡與持續進化。
  - **METRICS（可觀測性指標，migration v21）**：記錄「工具如何健康運行」。於底層插樁 `mcp_tool_metrics`，實時收集每次 MCP 工具調用的響應延遲、執行狀態、呼叫者與 Error Class，並藉由 `v_tool_perf_30d` 視圖輸出 30 天內效能與 Rate Limit 診斷指標，為 AI Agent 的工具選擇與調優提供客觀的數據量化支撐。

  這三者透過 `analysis_id`、`tool_id` 與 `metric_id` 形成高度關聯的知識圖譜，構成可追溯、可追蹤、可推導的完整生資實驗室大腦。
