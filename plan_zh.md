...
├── IMPLEMENTATION_PLAN.md        ← 當前 Phase 執行計畫（驗收標準）
├── execution_trace.md            ← Phase 執行紀錄（含 commit hash）
├── plan_zh.md                    ← 本計畫文件
├── plan.md                       ← 英文版
...
│
├── scripts/                      ← 一次性資料轉換工具
│   ├── 00_init_db.py             ← ✅ 建立所有表（含 analysis_history + tool_id 預留）
│   ├── 01_register_sample.py     ← 自動掃描 + 登記 L3 樣本
│   ├── 02_spatial_to_parquet.py  ← ✅ L3 Visium HD → L2 Parquet（已驗證）
│   └── msseg/                    ← MSseg 工具腳本
│
├── analysis/                     ← 分析函式庫（持續擴充）
│   ├── spatial_eda.py
...
│   └── candidates/               ← 待審區（Code Promotion 升格草稿）
│
├── server/                       ← MCP Server
│   └── bio_memory_server.py      ← 含 bio_history_* 工具
│
├── scheduler/                    ← 排程任務（launchd / cron 驅動）
│   ├── backup_db.py              ← ✅ 每日 EXPORT DATABASE 備份
│   ├── daily_qc.py               ← Phase 6
│   └── sample_watcher.py         ← Phase 6
│
├── docs/                         ← 操作指南與排程範本
│   ├── L3_DATA_INGEST_GUIDE.md
│   ├── TEST_DATABASE_INDEX.md
│   └── launchd_backup.plist.example
│
├── reports/                      ← 學術作業報告（PartA/PartB，.pdf/.docx gitignored）
│
├── tests/                        ← 測試套件
│   ├── conftest.py
│   ├── test_init_db.py
...
#### sandbox_exec() 實作策略（分階段）

`sandbox_exec()` 的隔離強度視部署階段而定：

| 階段 | 隔離方式 | 說明 |
|------|---------|------|
| macOS 測試（現階段） | `subprocess.run` + `cwd` 白名單 + timeout | 簡單夠用；信任 Claude 生成 + ALLOWED_IMPORTS 過濾 |
| Linux 部署（Phase 5+） | Docker container（`python:3.11-slim` + bind-mount silver/） | 強隔離；container 啟動 ~2 秒可接受 |
| 高安全需求（未來） | `nsjail` / `firejail`（Linux only） | 細粒度 syscall 限制；macOS 不適用 |

```python
# server/code_executor.py — macOS 測試版本
...
def sandbox_exec(code: str, timeout: int = 60) -> ExecResult:
    ...
    try:
        r = subprocess.run(
            [sys.executable, path],
            cwd=SANDBOX_CWD,           # 白名單工作目錄
            timeout=timeout,
            capture_output=True, text=True,
        )
        return ExecResult(success=r.returncode == 0,
                          output=r.stdout, traceback=r.stderr)
    finally:
        os.unlink(path)
```

> 切換到 Docker 時只需替換 `sandbox_exec` 內部，外部介面不變。
...
DuckDB 單寫者模型 + 多人同時用 Telegram Bot 的情境：

```python
# server/agent.py — 寫入任務序列化
import asyncio
_write_lock = asyncio.Lock()

async def handle_message(user_msg: str, user_id: str) -> str:
    # 讀操作（SQL 查歷史、L1 命中）併行；寫入透過 lock 序列化
    if needs_write(user_msg):
        async with _write_lock:
            return await run_with_history_write(user_msg, user_id)
    return await run_read_only(user_msg)
```

- **讀操作**：每個請求開 `read_only=True` 連線，併行不受限
- **寫入操作**（`analysis_history` / `sample_registry` / `memory_recent`）：透過 `asyncio.Lock` 串行
- **L3 長任務**（~4 小時 SpaceRanger）：寫一筆 `running` 記錄後立刻釋放 lock，背景跑；完成時 acquire lock 再 UPDATE
...
def cleanup_stale_runs(con: duckdb.DuckDBPyConnection, hours: int = 24) -> int:
    ...
    # DuckDB 沒有 result.rowcount，必須先 COUNT 再 UPDATE
    cleaned = con.execute(f"""
...
> 實際實作見 [config/db_utils.py](config/db_utils.py)。

### 目錄與排程

```
scheduler/
├── backup_db.py            ← 每日 02:00 EXPORT DATABASE（已驗證 round-trip）
├── cleanup_l1_cache.py     ← 每日 03:30 DELETE memory_recent WHERE expires_at < now()
├── rebuild_hnsw.py         ← 每週日 03:00 HNSW 索引重建
├── daily_qc.py             ← Phase 6：每日新樣本 QC
└── sample_watcher.py       ← Phase 6：新樣本自動登記
...
- [x] **執行** `uv run python scripts/00_init_db.py` 驗證 Schema 實際可建立
- [x] 填入 CRC 官方數據至 `sample_registry`（測試樣本：`crc_official_v4` 等 4 筆）
- [x] 建立 `analysis_history` 主表與 `analysis_index` 視圖（含 `tool_id UUID` 預留欄位）
...
- 格式 `{project_short}_{sample_short}`（全小寫、底線分隔，避免空格與大小寫混用）
- 範例：`crc_official_v4`、`mq250428_d1_d2`、`kallisto_v1`
- L3 目錄名稱可保留原始格式（如 `crc_visium_data/official_v4/`），sample_id 由腳本標準化後寫入
...
- [x] 撰寫 `scripts/02_spatial_to_parquet.py`（使用 CRC 官方數據測試）
- [x] 輸出：`silver/spatial_counts_crc_official_v4_8um/`（chunked, 104 parts, 416 MB）
- [x] 輸出：`silver/spatial_meta_crc_official_v4.parquet`（516,880 bins）
- [x] 驗證 DuckDB 可依基因名稱與空間座標查詢
...
L3 來源：`bulk_rna_data/Kallisto_v1/results_kallisto/`

**✅ 已完成（2026-05-16）：**

- [x] `scripts/bulk_rna/kallisto_to_matrix.py` — 彙整 `abundance.tsv`，輸出 transcript-level 矩陣
- [x] `scripts/bulk_rna/merge_kallisto_to_gene_matrix.py` — 彙整成 gene-level counts/TPM TSV
- [x] `scripts/bulk_rna/merge_kallisto_to_ensembl_matrix.py` — 合併為 Ensembl gene ID 矩陣
- [x] `scripts/bulk_rna/map_ensembl_to_symbol.py` — ENSMUSG ID → gene symbol 對照轉換
- [x] `scripts/bulk_rna/export_deseq2_counts.py` — 輸出整數 raw counts + colData 供 DESeq2/edgeR
- [x] `scripts/01_register_sample.py` — 自動掃描 `results_kallisto/` 並登記至 `sample_registry`
- [x] `analysis/bulk_eda.py` — QC 統計、Top genes、Pearson 相關矩陣、PCA 圖、EDA 報告

**待完成：**

- [ ] 輸出：`silver/bulk_counts_kallisto_v1.parquet`（L2 Parquet 轉換腳本）
...
- `bio_run_bulk_eda` — 觸發完整 EDA，回傳 `analysis_id` + `report_path`，並寫入 `analysis_history`
...
CREATE TABLE memory_recent (
    ...
    embedding   FLOAT[1536],     -- 向量嵌入（Gemini 3072-dim 降維至 1536）
    created_at  TIMESTAMP DEFAULT now(),
    expires_at  TIMESTAMP        -- TTL 7 天
);

-- HNSW 向量索引
CREATE INDEX memory_recent_emb_idx ON memory_recent USING HNSW (embedding)
WITH (metric = 'cosine');
```

### HNSW 索引維護（必要排程）

DuckDB VSS 的 HNSW 索引**不支援 incremental update**：INSERT/DELETE 後查詢仍可運作，
但召回率與效能會隨累積寫入逐步退化，必須定期重建。

```python
# scheduler/rebuild_hnsw.py（每週日 03:00 排程）
def rebuild_hnsw(con):
    con.execute("DROP INDEX IF EXISTS memory_recent_emb_idx")
    con.execute(
...
觸發時機：(a) 每週固定重建；(b) `memory_recent` 累積寫入 ≥ 100 筆後手動觸發。

### L1 TTL 清理（必要排程）

`memory_recent` 有 `expires_at` 但**沒人自動 DELETE**——必須加上排程任務：

```python
# scheduler/cleanup_l1_cache.py（每日 03:30，HNSW 重建後）
def cleanup_expired(con):
    deleted = con.execute("SELECT COUNT(*) FROM memory_recent WHERE expires_at < now()").fetchone()[0]
    if deleted:
...
> 刪除大量 row 後配合 HNSW 重建，索引才會真正釋放空間。

**嵌入模型：✅ 已決定 — Google Gemini Embedding**

| 屬性 | 值 |
|------|------|
| 模型 | `gemini-embedding-001`（主），fallback `text-embedding-004` |
| 維度 | 1536（從原生 3072 降維，與 schema 對齊） |
| 多語 | ✅ 中英混雜（基因名英文 + 描述中文）表現佳 |
| 計費 | 免費額度（5 RPM / 100 RPD），超出後付費（成本仍低於 OpenAI） |
| API key | `GOOGLE_API_KEY`（.env） |

**為何不選 OpenAI / 本地模型：**

- OpenAI `text-embedding-3-small`：可用，但已使用 Claude API，多一個雲端依賴沒必要
- 本地 `nomic-embed-text`：中文能力弱（訓練語料英文為主）
- 本地 `bge-m3`：中文佳，但需 ~2 GB RAM 常駐，macOS 測試階段不划算

**降維策略**：Gemini 原生 3072-dim，存入 DuckDB 前用 `[:1536]` 截斷或 PCA 降至 1536-dim，可避免 `memory_recent.embedding` 過大（每筆 12 KB → 6 KB）。
...
以 macOS launchd（測試階段）或 Linux cron（生產）驅動，不依賴複雜框架。

### ✅ 已建置排程（2026-05-16）

| 腳本 | 頻率 | 功能 | plist 範本 |
| ---- | ---- | ---- | --------- |
| `scheduler/backup_db.py` | 每日 02:00 | EXPORT DATABASE 備份至 APFS | `docs/launchd_backup.plist.example` |
| `scheduler/cleanup_l1_cache.py` | 每日 03:30 | 清理 L1 TTL 過期記錄 | `docs/launchd_cleanup_l1.plist.example` |
| `scheduler/rebuild_hnsw.py` | 每週日 03:00 | 重建 HNSW 向量索引 | `docs/launchd_rebuild_hnsw.plist.example` |
| `scheduler/scan_new_samples.py` | 每 30 分鐘 | 掃描 `results_kallisto/` 並登記新樣本 | `docs/launchd_scan_samples.plist.example` |

**啟用 scan 排程：**

```bash
cp docs/launchd_scan_samples.plist.example \
   ~/Library/LaunchAgents/com.hermes.scan_samples.plist
launchctl load ~/Library/LaunchAgents/com.hermes.scan_samples.plist
```

### 待實作排程

#### Phase B — FASTQ 自動 Kallisto 觸發（Linux 伺服器）

- 偵測 `results_kallisto/` 新增 FASTQ → 自動提交 Kallisto job
- 完成後依序：L2 pipeline → `sample_registry` 登記 → `analysis_history` 寫入

#### Phase C — 通知整合

- Kallisto / L2 pipeline 完成 → Telegram 推送結果摘要
- 每日 09:00：對新完成樣本產生 QC 報告並推送

**Linux cron 設定範例（生產部署）：**

```bash
*/30 * * * * python /mnt/space4/bio_lab_db/scheduler/scan_new_samples.py
0 2  * * * python /mnt/space4/bio_lab_db/scheduler/backup_db.py
30 3 * * * python /mnt/space4/bio_lab_db/scheduler/cleanup_l1_cache.py
0 3  * * 0 python /mnt/space4/bio_lab_db/scheduler/rebuild_hnsw.py
```
...
## Linux 伺服器遷移 Checklist

本機 macOS 測試完成後，遷移至 `/mnt/space4/bio_lab_db/` 依序執行：

1. **環境準備**
   - [ ] 在 Linux 上以 `uv` 建立 venv（不需 symlink workaround，APFS/ext4 都支援）
   - [ ] `uv sync --no-install-project` 安裝依賴
   - [ ] `INSTALL vss; LOAD vss;` 驗證 HNSW 擴充可用

2. **資料遷移**
   - [ ] `bio_memory.duckdb`：用 `EXPORT DATABASE` 在 macOS 匯出 → rsync → 在 Linux `IMPORT DATABASE`（避開直接拷貝 .duckdb 跨檔系統的風險）
   - [ ] L2 Parquet（`silver/`）：rsync 整個目錄（~500 MB+）
   - [ ] L3 原始數據：依使用情境決定全部 rsync 或按需傳輸

3. **路徑與設定**
   - [ ] 更新 `.env`：`BIO_DB_ROOT`、`L3_DATA_ROOT`、`DUCKDB_PATH` 改為 `/mnt/space4/...`
   - [ ] `config/settings.py` 自動讀 `BIO_DB_ROOT`，無需改程式碼

4. **排程遷移**
   - [ ] launchd plist → systemd timer 或 cron
   - [ ] 備份目標路徑改至 Linux 本地有日誌的檔系統（如 `/var/backups/bio_db/`）

5. **驗證**
   - [ ] 跑 `db_health_check()` 對比 macOS 與 Linux 計數一致
   - [ ] 重新建立 HNSW 索引（若先前已有 L1 cache）
   - [ ] 跑一次完整查詢（Telegram → Agent → L1/L2 → 回覆）
...
目前只有 `test_init_db.py`。建議在 Phase 2B 前補：

- `tests/test_db_utils.py`：`safe_write` / `cleanup_stale_runs` / `db_health_check`
- `tests/test_backup.py`：backup → restore round-trip（已驗證可行，正式化為自動測試）
- `tests/test_spatial_to_parquet.py`：小樣本端到端轉換正確性

### Schema 細節釐清

- `spatial_meta.cluster_id` / `cell_type` 應允許 NULL（clustering 是後續分析回填，不是 L2 轉換時就有）
- `bulk_counts.condition` / `replicate` 來自 `samples.csv`（Kallisto 慣例），由 `02_kallisto_to_parquet.py` 讀取後寫入
- `analysis_history.summary` 長度限制改為「≤ 100 chars 或 ≤ 60 token」（中英混雜）

### 圖檔產出 .gitignore

`results/` 隨分析累積會塞滿 git，必須加入 .gitignore：

```
# 分析產出圖檔（路徑記錄於 analysis_history.result_path）
results/
```

### analysis_msseg/ 定位

`analysis_msseg/`、`backend_msseg/` 為**參考用途**（不會被改、不會被 import），等 `analysis/` 的對應功能完成後，可由開發者自行決定是否移除以節省空間。

---

## 已決定事項

- **Agent 框架**：自製輕量 Agent + Claude API（不採用 Hermes — 需 GPU 自架，規模不符）
- **資料庫引擎**：DuckDB + VSS（HNSW）
- **Embedding 模型**：Google `gemini-embedding-001`（多語、含中文，免費額度 + 付費延伸）
- **Code Generation 沙盒**：macOS 測試階段用 `subprocess` + 工作目錄白名單；Linux 部署改 Docker container
- **多人並發**：寫入經 `asyncio.Lock` 串行；讀操作走 `read_only=True` 連線併行
- **L2 解析度**：8µm bins（2µm 按需從 L3 載入，不持久化）
- **備份策略**：每日 02:00 `EXPORT DATABASE` 至 `~/bio_db_backups/`（APFS），保留 7 天；已驗證 round-trip
- **ExFAT 防護**：關鍵寫入後 CHECKPOINT + 殭屍狀態（>24h running）清理
- **HNSW 維護**：每週日 03:00 重建索引；L1 TTL 清理每日 03:30
- **sample_id 命名**：`{project_short}_{sample_short}` 全小寫底線（如 `crc_official_v4`）
- **tool_id 預留**：`analysis_history.tool_id UUID` 欄位已加入（NULL allowed），等規模到了直接填入 tools 表外鍵

## 待解決問題

1. **訊息平台確認**：Telegram / LINE / Slack — 依實驗室成員習慣決定，影響第零階段實作
2. **launchd 排程啟用**：backup / cleanup / rebuild_hnsw / scan_samples 四個 plist 範本均已備妥，待 `launchctl load` 正式啟用
3. **Linux 伺服器權限**：確認 `/mnt/space4/` 有足夠磁碟空間與寫入權限
4. **MQ250422-A1-D1 缺失檔案**：web_summary 等檔案不存在（SpaceRanger 既有問題）— 以 D1-D2 為主要原型
5. **NDPI 配準**：兩個 MQ250428 樣本的高解析影像尚未對齊 — 完成前空間圖無組織影像疊加
