...
**ENGRAM（Evidence & iNdexed Graph of Research Artifacts & Memory）** 是本系統原創設計的**分析產出永久記憶系統**，與 HELIX 共同構成雙軌記憶架構。每次分析完成後，所有輸出檔案（圖、CSV、報告）透過 `register_artifact()` 寫入 `analysis_artifacts` 表，自動讀取 MIME、生成語意 embedding；≤ 500 KB 的檔案同時存入 `analysis_artifact_blobs`（migration v14 blob 分離，消除 HNSW 掃描寬列懲罰）。搜尋採 **RRF 混合搜尋**（migration 9A-2）：Layer 1 精確 subtype SQL 與 Layer 2 HNSW cosine 同時執行，透過 Reciprocal Rank Fusion（k=60）融合排名，結果附 `score`（RRF 分數，~0.008–0.033）與 `search_layer`（'exact'/'hnsw'/'rrf'）；每次搜尋自動寫入 `engram_search_metrics` 供長期調優分析（migration v15）。ENGRAM Web UI（`/engram`）提供樣本側邊欄、分析卡片、縮圖格狀佈局、Lightbox 放大、subtype 篩選、多選並排比較與語意搜尋，讓實驗室成員不需任何程式能力即可瀏覽全部分析的視覺輸出歷史。詳細論述見**附錄 A8**。
...
| ENGRAM-Core（產出記憶） | `analysis/artifact_registry.py`           | register_artifact / get_artifacts / compare_analyses / artifact_summary / search_artifacts（兩層搜尋） |
| ENGRAM-Web（瀏覽介面）  | `server/static/engram.html` + 8 個 API   | 樣本側邊欄、縮圖格狀佈局、Lightbox、subtype 篩選、多選並排比較、語意搜尋    |
...
L2 銀層（Silver）── 結構化特徵儲存 + 雙軌記憶系統
    ...
    bio_memory.duckdb        ← sample_registry + analysis_history + Views
      ...
      └─ [ENGRAM] analysis_artifacts + HNSW cosine 索引
...
HELIX 與 ENGRAM 是同一主資料庫（`bio_memory.duckdb`）內的兩個互補記憶系統，各自解決不同維度的「記憶遺失」問題，但共享同一組基礎設施（DuckDB、HNSW、embedding server）。

```
bio_memory.duckdb
│
├─ HELIX（工具記憶）                  ├─ ENGRAM（產出記憶）
│   tools                             │   analysis_artifacts
│   tool_change_log                   │   ├─ 圖（pca / volcano / heatmap...）
│   tool_stabilization_log            │   ├─ CSV（deg_list / qc_csv...）
│   │                                 │   └─ 報告（eda_report / summary_report）
│   記憶對象：工具本身的演變           │
│   「這個工具改了幾次、為何改、       │   記憶對象：每次分析的輸出結果
│    改完有沒有變好」                   │   「這次分析產出了哪些圖、在哪裡、
│                                     │    和上次相比有什麼不同」
│   查詢方式：                         │
│   - bio_tool_health report          │   查詢方式：
│   - trend / stabilize / prune      │   - /engram Web UI 直接瀏覽
│   - 視覺快照（VLM 100 token）        │   - /api/engram/search（語意）
│                                     │   - /api/engram/compare（並排）
```

| 維度 | HELIX | ENGRAM |
| --- | --- | --- |
| 記憶對象 | 工具程式碼的版本與健康 | 分析輸出的檔案與視覺結果 |
| 主資料表 | `tools` + `tool_change_log` | `analysis_artifacts` |
| 向量搜尋 | 無（SQL 精確查詢） | RRF 混合搜尋（exact + HNSW cosine，k=60） |
| 視覺展示 | 640×640 PNG 快照（VLM 讀取） | blob base64（瀏覽器直接渲染，v14 分離） |
| 時間衰減 | Ebbinghaus 降採樣（6 個月、1 年） | 無衰減（永久保存原始 base64） |
| 主要使用者 | Agent / 開發者（工具維護） | 實驗室成員（結果瀏覽與比較） |
| 寫入觸發 | 開發者呼叫 `register_tool()` | 分析完成後自動呼叫 `register_artifact()` |

兩軌記憶在「跨版本結果比較」場景交匯：透過 `analysis_history.tool_id`，ENGRAM 的每個 artifact 都能追蹤回 HELIX 管理的工具版本，`compare_analyses()` 函數在並排比較時自動附上 `tool_version` 與 `tool_status`，讓使用者在看圖的同時就能知道「這張圖是哪個版本的工具算出來的、那個工具現在是否還 active」。
...
| `l1_cache.py`               | `write_to_l1_cache()`, `search_cache()`, `invalidate_tool_cache()`                        | ✅   |
| `artifact_registry.py`      | `register_artifact()`, `get_artifacts()`, `compare_analyses()`, `artifact_summary()`, `search_artifacts()` — ENGRAM-Core | ✅   |
| `tool_registry.py`          | `register_tool()`, `get_hot_tools()`, `get_hot_lines()`, `open_stabilization()`, `close_stabilization()`, `mark_stable()` — HELIX-Core | ✅   |
| `tool_visualizer.py`        | `render_diagnosis_snapshot()`, `downsample_snapshot()`, `compute_loc()`, `compute_halstead_volume()` — HELIX-Vision | ✅   |
...
| 測試覆蓋  | 191/194 PASSED（3 skipped 為既有路徑問題，非程式碼問題）                                 |

### 實作階段摘要

Phase 1–8 已全數完成，涵蓋 Schema 建置、L2 Parquet 轉換、分析函式庫、L1 語意快取、Agent Loop、排程系統、雙推理後端、Web UI 多模態。

**雙軌記憶系統已全面建立：**

- **HELIX**（工具記憶）：content-hash 版本化、工具層熱區偵測、行級 churn 分析（difflib + Nagappan）、穩定化迭代生命週期（open/close/mark_stable/auto_revert）、遺忘曲線視覺快照（640×640 PNG，Ebbinghaus 降採樣）。migration v7 + v8 已執行，47/47 tests PASSED。

- **ENGRAM**（產出記憶）：`analysis_artifacts` 表 + HNSW cosine 索引（migration v9）、五個 ENGRAM-Core 函數（register/get/compare/summary/search）、兩層搜尋（精確 subtype → HNSW fallback）、8 個 ENGRAM API 路由、`engram.html` Web UI（Lightbox / 並排比較 / 語意搜尋）。23/23 tests PASSED。
