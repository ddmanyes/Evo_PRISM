...
工具是分析記憶的「執行者」，其版本與健康狀態本身也是記憶的一部分，與三層架構緊密整合：
...
**閉環設計要點**：
- `report` action 自動偵測未處理熱區並附上可直接呼叫的 `stabilize` 參數
- 進行中迭代的 `diagnosis_img` 快照由 `report` 回傳給 VLM，無需重讀原始碼
- `trend` action 查看跨迭代 CC delta，評估整體工具健康走向
...
**來源**：
- Content-hash 版本化：靈感來自 Git blob 物件（SHA 指紋偵測靜默修改）
- Cyclomatic Complexity：McCabe（1976）；本系統透過 radon 函式庫計算
- 視覺記憶壓縮：DeepSeek-OCR Contexts Optical Compression（arXiv:2510.18234）；640×640 PNG ≈ 100 VLM vision tokens ≈ 文字版本的 1/10
- 遺忘曲線降解析度：Ebbinghaus 遺忘曲線概念類比；舊快照逐步降為 320×320 → 160×160，模擬記憶模糊而非直接刪除

**截取想法**：

- Git 以 SHA 指紋追蹤檔案變化；同理，工具源碼以 content-hash 追蹤，`revision_count` 是不穩定性的客觀計數器
- McCabe 的 CC 值是可量測的複雜度代理指標——穩定化迭代前後的 CC delta 是「是否真的改善」的客觀依據，避免只靠主觀描述
- VLM 讀圖比讀等量文字更省 token；將診斷快照以 PNG 儲存，讓未來的 Agent 迭代時可用視覺方式快速重建脈絡，而非重讀源碼
- 遺忘曲線的核心不是「刪除」而是「模糊」——降解析度快照保留了空間佈局資訊（哪裡密、哪裡稀疏），只犧牲細節，比直接刪除更接近生物記憶的運作方式

**本系統的原創設計**：

將四個層次整合為一個閉環：
1. **偵測**：`register_tool()` 計算 content-hash，revision_count 累積
2. **診斷**：`open_stabilization()` 渲染視覺快照，寫入 `diagnosis_img`
3. **驗收**：`close_stabilization()` 計算 `complexity_after`，delta 量化改善
4. **壓縮**：`downsample_snapshot()` 逐步降解析度舊快照，模擬遺忘曲線
5. **剪枝**：`prune_deprecated()` 穩定工具積極清理（保留 2 版），熱區保留歷史（保留 10 版）

`report` action 自動偵測未處理熱區並附上可直接呼叫的 `stabilize` 參數；`trend` action 查看跨迭代 CC 趨勢，兩者合力讓閉環在 Agent 對話中自驅運行，無需人工持續追蹤。
...
Phase 1–8 已全數完成，涵蓋 Schema 建置、L2 Parquet 轉換、分析函式庫、L1 語意快取、Agent Loop、排程系統、雙推理後端、Web UI 多模態。工具健康管理閉環（content-hash 版本化、熱區偵測、穩定化迭代、視覺記憶壓縮）亦已完成並整合至 agent。詳細各階段狀態見**十六章**。
