...
# 1. Embedding server (port 8081)
if curl -sf http://localhost:8081/health >/dev/null 2>&1; then
    warn "embedding server already running on port 8081 (not managed by this script — Ctrl+C will NOT stop it)"
else
...
echo ""
info "========================================"
info "  Hermes ready  (Claude API mode)"
...
if [ -n "$EMBED_PID" ] && [ -n "$WEB_PID" ]; then
    wait "$EMBED_PID" "$WEB_PID"
elif [ -n "$EMBED_PID" ]; then
    wait "$EMBED_PID"
elif [ -n "$WEB_PID" ]; then
    wait "$WEB_PID"
else
